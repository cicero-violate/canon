## Layer 0 — Mechanical (no LLM, deterministic)

These are pure text/AST transforms. Fast, replayable, zero API cost.
...
## Layer 1 — Template-guided (LLM writes body only)

The structure is known. The LLM fills in logic inside a bounded scope. One call, one unit.
...
## Layer 2 — Reasoning required (full LLM call, spawned by failure or ambiguity)

Never in the initial plan. Only appear as recovery nodes in the tlog, spawned by Layer 0/1 failures or explicit refactor goals.
...
## Summary
The important property is that **Layer 3 is what gives your tlog its self-healing shape**. `CargoCheck` and `RebuildRepomap` are tasks too — they appear as nodes in the tlog, they have outcomes, and their outcomes spawn children. The execution tree isn't just code edits — it's the full causal history of the agent's decisions.
