...
impl ProjectEditor {
    ...
    pub fn load_with_rustc(_project: &Path) -> Result<Self> {
        anyhow::bail!("rustc-backed loading removed from rename crate")
    }
    ...
    pub fn validate(&self) -> Result<Vec<EditConflict>> {
        ...
        for (symbol_id, _handle) in &self.registry.handles {
            if self.oracle.is_macro_generated(symbol_id) {
                conflicts.push(EditConflict { symbol_id: symbol_id.to_string(), reason: "symbol generated by macro".to_string() });
            }
        }
