        ...
        "impl" => {
            ...
            ast.items.iter().find_map(|i| match i {
                syn::Item::Impl(imp) => {
                    ...
                    // Do not filter impl blocks by `name`.
                    // Impl blocks do not have a stable identifier; selection
                    // must rely on span (start_line) and/or trait matching only.
...
pub(crate) fn rebuild_graph_snapshot(project_root: &Path) -> Result<GraphSnapshot> {
    let _ = project_root;
    anyhow::bail!("graph rebuild removed from rename crate")
}
