(async function modifyAndSendMessage({
  text = "Hello from MMSB ✅",
  pollInterval = 150,
  timeout = 60000
} = {}) {
  const sleep = (ms) => new Promise(res => setTimeout(res, ms));
  console.log("[MMSB ChatGPT Submit] SCRIPT STARTED, text length:", text.length);

  // ========== SET PENDING INJECTION ==========
  window.__mmsbPendingPrompt = text;
  console.log('[MMSB Submit] Set pending prompt:', text.length, 'chars', 'First 100 chars:', text.substring(0, 100));
  // ===========================================
  ...
  if (!E) { console.log('[MMSB Submit] NO_INPUT'); return { ok: false, reason: "NO_INPUT" }; }

  // Fill with placeholder (request hook swaps this at send time)
  const placeholder = "<PROMPT>";
  console.log('[MMSB Submit] Filling editable with placeholder');
  ...
  console.log('[MMSB Submit] Placeholder applied, waiting for send button');
  await sleep(500);
  ...
  while (performance.now() - start < timeout) {
    ...
    if (B && !B.disabled) {
      console.log('[MMSB Submit] Clicking send button');
      B.click();
      console.log("✅ Message sent via MMSB CDP");
      return { ok: true, sent: true };
    }
    await sleep(pollInterval);
  }

  console.log('[MMSB Submit] TIMEOUT_WAITING_FOR_BUTTON');
