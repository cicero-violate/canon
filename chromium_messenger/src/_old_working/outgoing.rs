...
pub fn open_conversation_tab(
    chrome_port: u16,
    conversation_id: &str,
) -> Result<TargetDescriptor, Box<dyn std::error::Error>> {
...
pub fn open_url_tab(
    ...
    let tab: TargetDescriptor = match response {
        ...
        Err(ureq::Error::Status(405, _)) => {
            match ureq::post(&new_url).timeout(Duration::from_secs(2)).call() {
                Ok(resp) => resp.into_json()?,
...
fn open_conversation_tab_via_browser(
    chrome_port: u16,
    url: &str,
) -> Result<TargetDescriptor, Box<dyn std::error::Error>> {
    let version_url = format!("http://localhost:{}/json/version", chrome_port);
    let version: Value = ureq::get(&version_url)
        .timeout(Duration::from_secs(2))
        .call()?
        .into_json()?;
    let ws_url = version
        ...
        .ok_or("Missing browser WebSocket debugger URL")?;
    ...
    let start = std::time::Instant::now();
    while start.elapsed() < Duration::from_secs(2) {
        let targets: Vec<TargetDescriptor> = ureq::get(&list_url)
            .timeout(Duration::from_secs(2))
            .call()?
            .into_json()?;
        if let Some(tab) = targets
            .into_iter()
            .find(|target| target.id.as_deref() == Some(&target_id))
        {
            return Ok(tab);
        }
        std::thread::sleep(Duration::from_millis(100));
