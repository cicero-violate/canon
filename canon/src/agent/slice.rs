...
fn extract_field(ir: &CanonicalIr, field: &IrField) -> (String, Value) {
    match field {
        ...
        IrField::ModuleEdges => (
            "module_edges".into(),
            truncate_list(serde_json::to_value(&ir.module_edges).unwrap_or(Value::Null)),
        ),
        IrField::Structs => (
            "structs".into(),
        ...
        IrField::Functions => (
            "functions".into(),
            truncate_list(Value::Array(
                ir.functions.iter().map(slim_function).collect(),
            )),
        ),
        IrField::CallEdges => (
            "call_edges".into(),
            truncate_list(serde_json::to_value(&ir.call_edges).unwrap_or(Value::Null)),
        ),
        IrField::TickGraphs => (
            "tick_graphs".into(),
            truncate_list(serde_json::to_value(&ir.tick_graphs).unwrap_or(Value::Null)),
        ),
        IrField::SystemGraphs => (
            "system_graphs".into(),
            truncate_list(serde_json::to_value(&ir.system_graphs).unwrap_or(Value::Null)),
        ),
        IrField::LoopPolicies => (
            "loop_policies".into(),
            truncate_list(serde_json::to_value(&ir.loop_policies).unwrap_or(Value::Null)),
        ),
        IrField::Ticks => (
            "ticks".into(),
        ...
        IrField::TickEpochs => (
            "tick_epochs".into(),
            truncate_list(serde_json::to_value(&ir.tick_epochs).unwrap_or(Value::Null)),
        ),
        IrField::PolicyParameters => (
            "policy_parameters".into(),
            truncate_list(serde_json::to_value(&ir.policy_parameters).unwrap_or(Value::Null)),
        ),
        IrField::Plans => (
            "plans".into(),
        ...
        IrField::Executions => (
            "executions".into(),
            truncate_list(serde_json::to_value(&ir.executions).unwrap_or(Value::Null)),
        ),
        IrField::Admissions => (
            "admissions".into(),
            truncate_list(serde_json::to_value(&ir.admissions).unwrap_or(Value::Null)),
        ),
        IrField::AppliedDeltas => (
            "applied_deltas".into(),
            truncate_list(serde_json::to_value(&ir.applied_deltas).unwrap_or(Value::Null)),
        ),
        IrField::GpuFunctions => (
            "gpu_functions".into(),
            truncate_list(serde_json::to_value(&ir.gpu_functions).unwrap_or(Value::Null)),
        ),
        IrField::Proposals => (
            "proposals".into(),
        ...
        IrField::JudgmentPredicates => (
            ...
            truncate_list(serde_json::to_value(&ir.judgment_predicates).unwrap_or(Value::Null)),
        ),
        IrField::Deltas => (
            "deltas".into(),
        ...
        IrField::Dependencies => (
            "dependencies".into(),
            truncate_list(serde_json::to_value(&ir.dependencies).unwrap_or(Value::Null)),
        ),
        IrField::FileHashes => (
            "file_hashes".into(),
            truncate_list(serde_json::to_value(&ir.file_hashes).unwrap_or(Value::Null)),
        ),
        IrField::RewardDeltas => (
            "reward_deltas".into(),
            truncate_list(serde_json::to_value(&ir.reward_deltas).unwrap_or(Value::Null)),
        ),
        IrField::WorldModel => (
            "world_model".into(),
            truncate_list(serde_json::to_value(&ir.world_model).unwrap_or(Value::Null)),
        ),
        IrField::GoalMutations => (
            "goal_mutations".into(),
            truncate_list(serde_json::to_value(&ir.goal_mutations).unwrap_or(Value::Null)),
        ),
    }
}
