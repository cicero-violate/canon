...
use crate::ir::{SystemState, Module};
use thiserror::Error;
use super::{enforce_routing_completeness, layout_node_key, FileTopology, LayoutNode};
#[derive(Debug, Error)]
...
pub fn validate_layout(
    ...
    let assigned_nodes = validate_routing(
        ir,
        layout,
        &file_lookup,
        &module_maps,
...
fn validate_modules_and_files(
    ...
    let module_lookup: HashMap<&str, &Module> = ir
        .modules
        .iter()
        .map(|m| (m.id.as_str(), m))
        .collect();
    ...
    for module in &layout.modules {
        if !seen_layout_modules.insert(module.id.as_str()) {
            violations
                .push(format!("layout module `{}` appears more than once", module.id));
        }
        if !module_lookup.contains_key(module.id.as_str()) {
            violations
                .push(format!("layout module `{}` does not exist in Canon", module.id));
        }
        for file in &module.files {
            if file_lookup.insert(file.id.as_str(), module.id.as_str()).is_some() {
                violations
                    .push(format!("file `{}` is attached to multiple modules", file.id));
            }
            for entry in &file.use_block {
                if entry.trim().is_empty() {
                    violations
                        .push(
                            format!(
                                "file `{}` contains an empty use_block entry", file.id
                            ),
                        );
                }
            }
...
fn build_file_lookup(layout: &FileTopology) -> HashMap<&str, &str> {
    let mut file_lookup: HashMap<&str, &str> = HashMap::new();
    for module in &layout.modules {
        for file in &module.files {
...
fn build_module_maps(ir: &SystemState) -> ModuleMaps<'_> {
    ModuleMaps {
        struct_modules: ir
            .structs
            .iter()
    ...
    for assignment in &layout.routing {
        ...
        if !assigned_nodes.insert(key) {
            violations
                .push(
                    format!(
                        "node `{}` ({}) assigned more than once", node_id, node_label
                    ),
                );
        }
        let Some(module_for_file) = file_lookup.get(assignment.file_id.as_str()) else {
            violations
                .push(
                    format!(
                        "{} `{}` routed to unknown file `{}`", node_label, node_id,
                        assignment.file_id
                    ),
                );
            continue;
