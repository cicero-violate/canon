...
use super::{LayoutAssignment, LayoutFile, LayoutGraph, LayoutModule, LayoutNode, LayoutStrategy, SemanticGraph};
    ...
    fn plan(&self, semantic: &SemanticGraph) -> LayoutGraph {
        ...
        for module in &semantic.modules {
            ...
            modules.push(LayoutModule {
                ...
                files: vec![LayoutFile { id: file_id.clone(), path: "mod.rs".to_owned(), use_block: Vec::new() }],
                imports: Vec::new(),
            });
            route_all_nodes(semantic, module.id.as_str(), &file_id, "LAY-005: single-file strategy", &mut routing);
        }
...
impl LayoutStrategy for PerTypeLayoutStrategy {
    ...
    fn plan(&self, semantic: &SemanticGraph) -> LayoutGraph {
        ...
        for module in &semantic.modules {
            let mut files = vec![LayoutFile { id: default_file_id(module.id.as_str()), path: "mod.rs".to_owned(), use_block: Vec::new() }];
            ...
            for structure in semantic.structs.iter().filter(|s| s.module == module.id) {
                ...
                let file_id = ensure_named_file(&mut files, &mut named_files, module.id.as_str(), &slug, format!("{}.rs", slug));
                file_for_struct.insert(structure.id.as_str(), file_id.clone());
                routing.push(LayoutAssignment { node: LayoutNode::Struct(structure.id.clone()), file_id, rationale: "LAY-005: per-type strategy".to_owned() });
            }

            for enumeration in semantic.enums.iter().filter(|e| e.module == module.id) {
                let slug = slugify(enumeration.name.as_str());
                let file_id = ensure_named_file(&mut files, &mut named_files, module.id.as_str(), &slug, format!("{}.rs", slug));
                routing.push(LayoutAssignment { node: LayoutNode::Enum(enumeration.id.clone()), file_id, rationale: "LAY-005: per-type strategy".to_owned() });
            }

            for tr in semantic.traits.iter().filter(|t| t.module == module.id) {
                let slug = slugify(tr.name.as_str());
                let file_id = ensure_named_file(&mut files, &mut named_files, module.id.as_str(), &slug, format!("{}.rs", slug));
                routing.push(LayoutAssignment { node: LayoutNode::Trait(tr.id.clone()), file_id, rationale: "LAY-005: per-type strategy".to_owned() });
            }

            for block in semantic.impls.iter().filter(|b| b.module == module.id) {
                let file_id = file_for_struct.get(block.struct_id.as_str()).cloned().unwrap_or_else(|| default_file_id(module.id.as_str()));
                routing.push(LayoutAssignment { node: LayoutNode::Impl(block.id.clone()), file_id, rationale: "LAY-005: per-type strategy".to_owned() });
            }
            ...
            for function in semantic.functions.iter().filter(|f| f.module == module.id) {
                let file_id = if let Some(struct_id) = impl_to_struct.get(function.impl_id.as_str()) {
                    file_for_struct.get(struct_id).cloned().unwrap_or_else(|| default_file_id(module.id.as_str()))
                } else {
                    default_file_id(module.id.as_str())
                };
                routing.push(LayoutAssignment { node: LayoutNode::Function(function.id.clone()), file_id, rationale: "LAY-005: per-type strategy".to_owned() });
            }

            modules.push(LayoutModule { id: module.id.clone(), name: module.name.clone(), files, imports: Vec::new() });
        }
...
fn route_all_nodes(semantic: &SemanticGraph, module_id: &str, file_id: &str, rationale: &str, routing: &mut Vec<LayoutAssignment>) {
    for structure in semantic.structs.iter().filter(|s| s.module.as_str() == module_id) {
        routing.push(LayoutAssignment { node: LayoutNode::Struct(structure.id.clone()), file_id: file_id.to_owned(), rationale: rationale.to_owned() });
    }
    for enumeration in semantic.enums.iter().filter(|e| e.module.as_str() == module_id) {
        routing.push(LayoutAssignment { node: LayoutNode::Enum(enumeration.id.clone()), file_id: file_id.to_owned(), rationale: rationale.to_owned() });
    }
    for tr in semantic.traits.iter().filter(|t| t.module.as_str() == module_id) {
        routing.push(LayoutAssignment { node: LayoutNode::Trait(tr.id.clone()), file_id: file_id.to_owned(), rationale: rationale.to_owned() });
    }
    for block in semantic.impls.iter().filter(|b| b.module.as_str() == module_id) {
        routing.push(LayoutAssignment { node: LayoutNode::Impl(block.id.clone()), file_id: file_id.to_owned(), rationale: rationale.to_owned() });
    }
    for function in semantic.functions.iter().filter(|f| f.module.as_str() == module_id) {
        routing.push(LayoutAssignment { node: LayoutNode::Function(function.id.clone()), file_id: file_id.to_owned(), rationale: rationale.to_owned() });
    }
}

fn ensure_named_file(files: &mut Vec<LayoutFile>, named_files: &mut HashMap<String, String>, module_id: &str, slug: &str, path: String) -> String {
    if let Some(existing) = named_files.get(slug) {
        return existing.clone();
    }
    let file_id = format!("file.{}.{}", sanitize_module(module_id), slug);
    files.push(LayoutFile { id: file_id.clone(), path, use_block: Vec::new() });
...
fn sanitize_module(module_id: &str) -> String {
    module_id.chars().map(|c| if c.is_ascii_alphanumeric() { c.to_ascii_lowercase() } else { '_' }).collect()
}
...
impl LayoutStrategy for FlatNodeLayoutStrategy {
    ...
    fn plan(&self, semantic: &SemanticGraph) -> LayoutGraph {
        ...
        for module in &sorted_modules {
            ...
            // ---- STRUCTS ----
            let mut structs: Vec<_> = semantic.structs.iter().filter(|s| s.module == module.id).cloned().collect();
            structs.sort_by(|a, b| a.id.cmp(&b.id));

            for structure in structs {
                let slug = slugify(structure.name.as_str());
                let file_id = format!("file.{}.struct.{}", sanitize_module(module.id.as_str()), slug);
                files.push(LayoutFile { id: file_id.clone(), path: format!("struct_{}.rs", slug), use_block: Vec::new() });
                routing.push(LayoutAssignment { node: LayoutNode::Struct(structure.id), file_id, rationale: "LAY-ATOM-001".to_owned() });
            }

            // ---- ENUMS ----
            let mut enums: Vec<_> = semantic.enums.iter().filter(|e| e.module == module.id).cloned().collect();
            enums.sort_by(|a, b| a.id.cmp(&b.id));

            for enumeration in enums {
                let slug = slugify(enumeration.name.as_str());
                let file_id = format!("file.{}.enum.{}", sanitize_module(module.id.as_str()), slug);
                files.push(LayoutFile { id: file_id.clone(), path: format!("enum_{}.rs", slug), use_block: Vec::new() });
                routing.push(LayoutAssignment { node: LayoutNode::Enum(enumeration.id), file_id, rationale: "LAY-ATOM-001".to_owned() });
            }

            // ---- TRAITS ----
            let mut traits: Vec<_> = semantic.traits.iter().filter(|t| t.module == module.id).cloned().collect();
            traits.sort_by(|a, b| a.id.cmp(&b.id));

            for tr in traits {
                let slug = slugify(tr.name.as_str());
                let file_id = format!("file.{}.trait.{}", sanitize_module(module.id.as_str()), slug);
                files.push(LayoutFile { id: file_id.clone(), path: format!("trait_{}.rs", slug), use_block: Vec::new() });
                routing.push(LayoutAssignment { node: LayoutNode::Trait(tr.id), file_id, rationale: "LAY-ATOM-001".to_owned() });
            }

            // ---- IMPLS ----
            let mut impls: Vec<_> = semantic.impls.iter().filter(|b| b.module == module.id).cloned().collect();
            ...
            for block in impls {
                ...
                files.push(LayoutFile { id: file_id.clone(), path: format!("impl_{}.rs", slug), use_block: Vec::new() });
                routing.push(LayoutAssignment { node: LayoutNode::Impl(block.id), file_id, rationale: "LAY-ATOM-001".to_owned() });
            }

            // ---- FUNCTIONS ----
            let mut functions: Vec<_> = semantic.functions.iter().filter(|f| f.module == module.id).cloned().collect();
            ...
            for function in functions {
                ...
                files.push(LayoutFile { id: file_id.clone(), path: format!("fn_{}.rs", slug), use_block: Vec::new() });
                routing.push(LayoutAssignment { node: LayoutNode::Function(function.id), file_id, rationale: "LAY-ATOM-001".to_owned() });
            }
