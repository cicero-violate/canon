...
use crate::io_utils::{load_ir_from_file, load_ir_or_semantic_graph, load_layout, resolve_layout};
...
pub async fn execute_command(cmd: Command) -> Result<(), Box<dyn std::error::Error>> {
    match cmd {
        ...
        Command::Diagnose { ir } => {
            ...
            match validate_ir(&ir_doc) {
                ...
                Err(errors) => {
                    ...
                    println!("{} violation(s) → {} root cause(s):\n", errors.violations().len(), briefs.len());
        ...
        Command::RenderFn { ir, fn_id } => {
            let ir_doc = load_ir_from_file(&ir)?;
            let function = ir_doc.functions.iter().find(|f| f.id == fn_id).ok_or("unknown function")?;
        ...
        Command::Ingest { src, semantic_out, layout_out } => {
            ...
            let workspace_name = src.file_name().and_then(|s| s.to_str()).unwrap_or("workspace");
            ...
            let engine = MemoryEngine::new(MemoryEngineConfig { tlog_path: tlog_path.clone() })?;
            ...
            println!("MemoryEngine snapshot written to `{}`.", checkpoint_path.display());
        }
        Command::ObserveEvents { ir, output } => {
            ...
            println!("Observation written to `{}` ({} modules, {} functions, {} call edges).", output.display(), obs.totals.modules, obs.totals.functions, obs.totals.call_edges,);
        }
        Command::ExecuteTick { ir, tick, parallel } => {
            let mut ir_doc = load_ir_from_file(&ir)?;
            validate_ir(&ir_doc)?;
            let mut exec = TickExecutor::new(&mut ir_doc);
            let mode = if parallel { TickExecutionMode::ParallelVerified } else { TickExecutionMode::Sequential };
        ...
        Command::SubmitDsl { dsl, ir, layout, output_ir, materialize_dir } => {
            let ir_doc = load_ir_from_file(&ir)?;
            let layout_doc = if let Some(layout_path) = layout { load_layout(layout_path)? } else { LayoutGraph::default() };
            ...
            println!("DSL proposal accepted; evolved IR written to `{}`.", output_ir.display());
        }
        ...
        Command::ShowGraph { graph } => {
            ...
            if !cap_graph.edges.is_empty() {
                ...
                for edge in &cap_graph.edges {
                    println!("{:<20} → {:<20} {:>8.3}", edge.from, edge.to, edge.proof_confidence);
                }
            }
        }
        Command::RunAgent { ir, layout, graph, proposal, ir_out, ledger_out, graph_out, max_ticks, meta_tick_interval, policy_update_interval } => {
            ...
            let stats = run_agent(&mut ir_doc, &mut layout_doc, &mut cap_graph, seed_proposal, &config, &ws_server::spawn("127.0.0.1:8787".parse()?)).await?;
            println!("Agent run complete — {} ticks", stats.len());
            println!("{:<6} {:>8} {:>8} {:>10} {:>6} {:>6}", "tick", "nodes", "errors", "reward", "meta", "policy");
            println!("{}", "-".repeat(52));
            for s in &stats {
                println!(
                    "{:<6} {:>8} {:>8} {:>10} {:>6} {:>6}",
                    ...
                    s.nodes_called,
                    s.llm_errors,
                    s.pipeline_reward.map(|r| format!("{r:.4}")).unwrap_or_else(|| "-".to_string()),
        ...
        Command::GpuAnalyze { ir, execute } => {
            ...
            if candidates.is_empty() {
                ...
                for c in &candidates {
                    println!("  {} → {}  (fn: {} → {})", c.producer_gpu, c.consumer_gpu, c.producer_function, c.consumer_function,);
                }
                if execute {
                    let first = &candidates[0];
                    let gpu_fn = ir_doc.gpu_functions.iter().find(|g| g.id == first.producer_gpu).ok_or("producer gpu function not found")?;
                    let canon_fn = ir_doc.functions.iter().find(|f| f.id == gpu_fn.function).ok_or("backing function not found")?;
                    let bytecode = FunctionBytecode::from_function(canon_fn)?;
                    let program = generate_shader(gpu_fn, &bytecode).map_err(|e| -> Box<dyn std::error::Error> { e.into() })?;
        ...
        Command::ExecuteGraph { ir, graph_id, checkpoint } => {
            let ir_doc = load_ir_from_file(&ir)?;
            let engine = MemoryEngine::new(MemoryEngineConfig { tlog_path: checkpoint.with_extension("tlog") })?;
            let interp = SystemInterpreter::new(&ir_doc, &engine);
            let result = interp.execute_graph(&graph_id, Default::default())?;
            println!("ExecuteGraph OK — graph={} nodes_executed={} deltas_emitted={}", graph_id, result.node_results.len(), result.emitted_deltas.len(),);
        }
        Command::BootstrapGraph { graph_out, proposal_out, ir } => {
            let ir_doc = load_ir_or_semantic_graph(&ir)?;
            let target_id = ir_doc.modules.first().map(|m| m.id.as_str()).unwrap_or("module_0").to_string();
            ...
            println!("  graph:    {} nodes, {} edges → {}", graph.nodes.len(), graph.edges.len(), graph_out.display());
            println!("  proposal: target={} kind={:?} → {}", target_id, crate::agent::refactor::RefactorKind::SplitModule, proposal_out.display());
            println!();
