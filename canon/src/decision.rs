...
use crate::dot_import::{DotImportError, dot_graph_to_file_topology, dot_graph_to_imported_types, dot_graph_to_proposal, parse_dot};
...
pub fn auto_accept_fn_ast(
    ...
    working.proposals.push(crate::ir::Proposal {
        id: proposal_id.clone(),
        goal: crate::ir::ProposalGoal {
            id: fn_word.clone(),
            description: format!("AST update for function `{function_id}`."),
        },
        nodes: vec![crate::ir::ProposedNode {
            id: Some(trait_id.clone()),
            name: fn_word,
            module: Some(module_id),
            kind: crate::ir::ProposedNodeKind::Trait,
        }],
        apis: vec![crate::ir::ProposedApi {
    ...
    if let Some(function) = acceptance.ir.functions.iter_mut().find(|f| f.id == function_id) {
        if let Some(DeltaPayload::UpdateFunctionAst { ast, .. }) =
            working.deltas.iter().find(|d| d.id == delta_id).and_then(|d| d.payload.as_ref())
        {
