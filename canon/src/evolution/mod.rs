...
mod structural;
use crate::ir::{
    AdmissionId, AppliedDeltaRecord, SystemState, StateChange, DeltaId, DeltaKind,
    ChangePayload, JudgmentDecision,
};
use crate::runtime::delta_verifier::{ChangeVerifier, VerificationError};
...
pub fn apply_admitted_deltas(
...
) -> Result<SystemState, EvolutionError> {
    let snapshot = ChangeVerifier::create_snapshot(ir);
    ...
    match ChangeVerifier::verify_application(ir, &next, admission_ids) {
        Ok(_) => Ok(next),
...
fn assert_delta_is_applicable(delta: &StateChange) -> Result<(), EvolutionError> {
    if matches!(delta.payload, Some(ChangePayload::AddFunction { .. }))
