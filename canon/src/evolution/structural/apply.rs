...
use crate::ir::{
    CanonicalIr, DeltaPayload, ImplBlock, Module, ModuleEdge, Struct, StructKind, Trait, Visibility,
};
...
pub fn apply_delta_payload(
    ...
    match payload {
        ...
        DeltaPayload::UpdateStructVisibility {
            struct_id,
            visibility,
        } => {
            let structure = ir
        ...
        DeltaPayload::RemoveField {
            ...
            if let Some(structure) = ir.structs.iter_mut().find(|s| s.id == *struct_id) {
                structure
                    .fields
                    .retain(|f| f.name.as_str() != field_name.as_str());
            }
        }
        DeltaPayload::RenameArtifact {
            kind,
            old_id,
            new_id,
        } => match kind.as_str() {
        ...
        DeltaPayload::AttachExecutionEvent {
            execution_id,
            event,
        } => {
            let record = ir
        ...
        DeltaPayload::UpdateFunctionInputs {
            function_id,
            inputs,
        } => {
            let function = ir
        ...
        DeltaPayload::UpdateFunctionOutputs {
            function_id,
            outputs,
        } => {
            let function = ir
