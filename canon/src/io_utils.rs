use crate::{
    layout::{FileTopology, ParsedModel},
    semantic_builder::SemanticIrBuilder, storage::reader::StateReader, SystemState,
};
use std::ffi::OsString;
use std::fs;
use std::path::{Path, PathBuf};
fn load_ir_from_path(path: &Path) -> Result<SystemState, Box<dyn std::error::Error>> {
    if path
        .extension()
        ...
        .unwrap_or(false)
    {
        return Ok(StateReader::read_ir_from_checkpoint(path)?);
    }
    let data = fs::read(path)?;
    if let Ok(ir) = serde_json::from_slice::<SystemState>(&data) {
        return Ok(ir);
    }
    let semantic: ParsedModel = serde_json::from_slice(&data)?;
...
pub fn load_layout(path: PathBuf) -> Result<FileTopology, Box<dyn std::error::Error>> {
    let data = fs::read(path)?;
