...
use crate::ir::{SystemState, GpuFunctionId};
...
pub fn analyze_fusion_candidates(ir: &SystemState) -> Vec<FusionCandidate> {
    let mut function_to_gpu: HashMap<&str, &crate::ir::GpuFunction> = HashMap::new();
    for gpu in &ir.gpu_functions {
        if gpu.properties.pure && gpu.properties.no_io && gpu.properties.no_alloc
    ...
    for edge in &ir.call_edges {
        ...
        if outgoing.get(producer_fn).map(|set| set.len() == 1).unwrap_or(false)
            && incoming.get(consumer_fn).map(|set| set.len() == 1).unwrap_or(false)
        {
            candidates
                .push(FusionCandidate {
