...
pub fn generate_shader(gpu: &GpuFunction, bytecode: &FunctionBytecode) -> Result<GpuProgram, String> {
    if gpu.outputs.len() != 1 {
        return Err(format!("gpu kernel `{}` must have exactly one output for now", gpu.id));
    }
    ...
    lines.push("@compute @workgroup_size(64) fn main(@builtin(global_invocation_id) id: vec3<u32>) {".into());
    ...
    for inst in &bytecode.instructions {
        match inst {
            Instruction::LoadInput(name) => {
                let idx = gpu.inputs.iter().position(|port| port.name.as_str() == name).ok_or_else(|| format!("input `{name}` missing in gpu kernel {}", gpu.id))?;
                let offset = input_offsets[idx];
                lines.push(format!("  stack[sp] = input_buffer[{offset}u + lane];"));
                lines.push("  sp = sp + 1u;".into());
            }
            Instruction::LoadConst(value) => {
                let literal = literal_to_f32(value).ok_or_else(|| format!("literal `{value:?}` unsupported in GPU kernels"))?;
            ...
            Instruction::LoadBinding(name) => {
                let var = binding_vars.get(name.as_str()).ok_or_else(|| format!("binding `{name}` referenced before store in gpu kernel {}", gpu.id))?;
            ...
            other => {
                return Err(format!("instruction `{other:?}` is not supported in GPU kernels"));
            }
        }
    }
    ...
    lines.push(format!("  output_buffer[{output_offset}u + lane] = result;"));
    lines.push("}".into());

    Ok(GpuProgram { shader: lines.join("\n"), workgroup_size: 64, lanes, input_offsets, output_offsets })
}

fn lane_count(gpu: &GpuFunction) -> Result<u32, String> {
    let source = gpu.inputs.first().or_else(|| gpu.outputs.first()).ok_or_else(|| format!("gpu kernel `{}` has no ports", gpu.id))?;
    ...
    for port in gpu.inputs.iter().chain(gpu.outputs.iter()) {
        if port.lanes != lanes {
            return Err(format!("gpu kernel `{}` mixes lane counts ({} vs {})", gpu.id, port.lanes, lanes));
        }
    }
    Ok(lanes)
}
...
fn compute_offsets(ports: &[VectorPort], lanes: u32) -> Vec<u32> {
    ports.iter().enumerate().map(|(idx, _)| (idx as u32) * lanes).collect()
}
...
pub fn flatten_ports(ports: &[VectorPort], data: &[Vec<f32>]) -> Result<Vec<f32>, String> {
    ...
    for (idx, port) in ports.iter().enumerate() {
        if port.lanes as usize != lanes {
            return Err(format!("port `{}` lanes mismatch (expected {lanes}, found {})", port.name, port.lanes));
        }
        if data[idx].len() != lanes {
            return Err(format!("port `{}` data length {} != lanes {}", port.name, data[idx].len(), lanes));
        }
    }
