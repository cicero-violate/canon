...
pub trait Executor {
    fn execute(&self, function: &Function, inputs: BTreeMap<String, Value>, context: &mut ExecutionContext) -> Result<BTreeMap<String, Value>, ExecutorError>;
}
...
impl<'a> FunctionExecutor<'a> {
    ...
    pub fn execute_by_id(&self, function_id: &FunctionId, inputs: BTreeMap<String, Value>, context: &mut ExecutionContext) -> Result<BTreeMap<String, Value>, ExecutorError> {
        let function = self.ir.functions.iter().find(|f| &f.id == function_id).ok_or_else(|| ExecutorError::UnknownFunction(function_id.clone()))?;
    ...
    pub fn execute_composition(&self, functions: &[FunctionId], initial_inputs: BTreeMap<String, Value>, context: &mut ExecutionContext) -> Result<Vec<BTreeMap<String, Value>>, ExecutorError> {
...
fn check_contract(function: &Function) -> Result<(), ExecutorError> {
    if !function.contract.total {
        return Err(ExecutorError::ContractViolation { function: function.id.clone(), reason: "function must be total".into() });
    }
    if !function.contract.explicit_inputs || !function.contract.explicit_outputs {
        return Err(ExecutorError::ContractViolation { function: function.id.clone(), reason: "inputs and outputs must be explicit".into() });
    }
    if !function.contract.effects_are_deltas {
        return Err(ExecutorError::ContractViolation { function: function.id.clone(), reason: "effects must be deltas".into() });
    }
    Ok(())
}

fn validate_inputs(function: &Function, inputs: &BTreeMap<String, Value>) -> Result<(), ExecutorError> {
    for port in &function.inputs {
        if !inputs.contains_key(port.name.as_str()) {
            return Err(ExecutorError::MissingInput { function: function.id.clone(), input: port.name.as_str().to_string() });
        }
        let value = &inputs[port.name.as_str()];
        if !value.is_compatible_with(&port.ty) {
            return Err(ExecutorError::TypeMismatch { function: function.id.clone(), port: port.name.as_str().to_string(), expected: format!("{:?}", port.ty), found: format!("{:?}", value.kind()) });
        }
    }
    Ok(())
}
