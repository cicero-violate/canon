...
use super::arith::{apply_arithmetic, ArithmeticOp};
use super::error::{ExecutorError, InterpreterError};
use super::Executor;

pub fn interpret_bytecode(
    executor: &dyn Executor, ir: &crate::ir::CanonicalIr, function: &Function, inputs: &BTreeMap<String, Value>, context: &mut ExecutionContext,
    ...
    for instruction in &program.instructions {
        match instruction {
            ...
            Instruction::LoadInput(name) => {
                let value = inputs.get(name).cloned().ok_or_else(|| InterpreterError::MissingInput { function: function.id.clone(), input: name.clone() })?;
                stack.push(value);
            }
            ...
            Instruction::LoadBinding(name) => {
                let key = binding_keys.get(name).ok_or_else(|| InterpreterError::BindingNotFound { function: function.id.clone(), binding: name.clone() })?;
                let value = context.lookup(key).map_err(InterpreterError::Context)?.clone();
                stack.push(value);
            }
            Instruction::FieldAccess(field) => {
                let value = pop(function, &mut stack)?;
                let struct_val = value.as_struct().map_err(|e| InterpreterError::TypeError { function: function.id.clone(), message: e.to_string() })?;
                let field_value = struct_val.get_field(field).map_err(|e| InterpreterError::TypeError { function: function.id.clone(), message: e.to_string() })?;
            ...
            Instruction::Call(target) => {
                let callee = ir.functions.iter().find(|f| &f.id == target).ok_or_else(|| InterpreterError::UnknownFunction { function: target.clone() })?;
                ...
                let outputs =
                    executor.execute(callee, owned_inputs, context).map_err(|e| InterpreterError::CallFailed { caller: function.id.clone(), callee: callee.id.clone(), source: Box::new(e) })?;
                for port in &callee.outputs {
                    let value = outputs.get(port.name.as_str()).ok_or_else(|| InterpreterError::MissingOutput { function: callee.id.clone(), output: port.name.as_str().to_string() })?;
    ...
    Err(InterpreterError::MissingReturn { function: function.id.clone() })
}

fn pop(function: &Function, stack: &mut Vec<Value>) -> Result<Value, InterpreterError> {
    stack.pop().ok_or_else(|| InterpreterError::StackUnderflow { function: function.id.clone() })
}

fn collect_outputs(function: &Function, stack: &mut Vec<Value>) -> Result<BTreeMap<String, Value>, InterpreterError> {
