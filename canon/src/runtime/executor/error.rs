...
pub enum ExecutorError {
    ...
    #[error("function `{function}` port `{port}` type mismatch: expected `{expected}`, found `{found}`")]
    TypeMismatch { function: FunctionId, port: String, expected: String, found: String },
    #[error("function `{function}` violates contract: {reason}")]
    ContractViolation { function: FunctionId, reason: String },
    #[error(transparent)]
...
pub enum InterpreterError {
    ...
    BindingNotFound { function: FunctionId, binding: String },
    #[error("function `{function}` missing output `{output}` from call")]
    MissingOutput { function: FunctionId, output: String },
    ...
    TypeError { function: FunctionId, message: String },
