...
use crate::runtime::ast::{compile_function_ast, FunctionAst};
...
impl FunctionBytecode {
    ...
    pub fn from_function(function: &Function) -> Result<Self, BytecodeError> {
        ...
        if let Some(ast_json) = function.metadata.ast.as_ref() {
            let ast: FunctionAst = serde_json::from_value(ast_json.clone()).map_err(|err| BytecodeError::InvalidAst { function: function.id.clone(), message: err.to_string() })?;
            let program = compile_function_ast(&ast).map_err(|err| BytecodeError::InvalidAst { function: function.id.clone(), message: err.to_string() })?;
            return Ok(program);
        }

        Err(BytecodeError::MissingBytecode { function: function.id.clone() })
    }

    pub fn decode(function: &Function, encoded: &str) -> Result<Self, BytecodeError> {
        let bytes = BASE64.decode(encoded).map_err(|err| BytecodeError::Decode { function: function.id.clone(), message: err.to_string() })?;
        bincode::deserialize(&bytes).map_err(|err| BytecodeError::Decode { function: function.id.clone(), message: err.to_string() })
    }

    pub fn encode(&self, function: &Function) -> Result<String, BytecodeError> {
        let bytes = bincode::serialize(&self.instructions).map_err(|err| BytecodeError::Encode { function: function.id.clone(), message: err.to_string() })?;
...
pub enum BytecodeError {
    ...
    Decode { function: FunctionId, message: String },
    #[error("function `{function}` failed to encode bytecode: {message}")]
    Encode { function: FunctionId, message: String },
    #[error("function `{function}` AST invalid: {message}")]
    InvalidAst { function: FunctionId, message: String },
}
