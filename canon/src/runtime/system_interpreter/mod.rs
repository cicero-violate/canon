...
use database::{AdmissionProof, CommitProof, JudgmentProof, MemoryEngine, MemoryEngineError, OutcomeProof};
...
impl<'a> SystemInterpreter<'a> {
    pub fn new(ir: &'a CanonicalIr, engine: &'a MemoryEngine) -> Self {
        Self { ir, executor: FunctionExecutor::new(ir), engine }
    }

    /// Execute a system graph by ID.
    pub fn execute_graph(&self, graph_id: &str, initial_inputs: BTreeMap<String, Value>) -> Result<SystemExecutionResult, SystemInterpreterError> {
        let graph = self.ir.system_graphs.iter().find(|g| g.id == graph_id).ok_or_else(|| SystemInterpreterError::UnknownGraph(graph_id.to_string()))?;

        self.run_graph(graph, initial_inputs)
    }

    /// Execute an inline system graph that isn't stored in the IR.
    pub fn execute_inline(&self, graph: &SystemGraph, initial_inputs: BTreeMap<String, Value>) -> Result<SystemExecutionResult, SystemInterpreterError> {
        self.run_graph(graph, initial_inputs)
    }

    fn run_graph(&self, graph: &SystemGraph, initial_inputs: BTreeMap<String, Value>) -> Result<SystemExecutionResult, SystemInterpreterError> {
        ...
        events.push(SystemExecutionEvent::Validation { check: "edges_declared", detail: format!("validated {} edges", graph.edges.len()) });
        let order = self.topological_sort(graph, &dependencies)?;
        events.push(SystemExecutionEvent::Validation { check: "topology", detail: format!("computed execution order of {} nodes", order.len()) });
        ...
        for node_id in &order {
            let node = node_index.get(node_id).copied().ok_or_else(|| SystemInterpreterError::UnknownNode(node_id.clone()))?;
            ...
            self.apply_node_effects(node, &outputs, &emitted_deltas, &mut proofs, &mut delta_provenance, &mut events)?;
            results.insert(node.id.clone(), outputs);
            events.push(SystemExecutionEvent::NodeExecuted { node_id: node.id.clone(), kind: node.kind.clone() });
        }
        ...
        Ok(SystemExecutionResult {
            ...
            delta_provenance,
            events,
            judgment_proof: engine_artifacts.as_ref().map(|artifacts| artifacts.judgment_proof.clone()),
            admission_proof: engine_artifacts.as_ref().map(|artifacts| artifacts.admission.clone()),
            commit_proofs: engine_artifacts.as_ref().map(|artifacts| artifacts.commits.clone()).unwrap_or_default(),
            outcome_proofs: engine_artifacts.as_ref().map(|artifacts| artifacts.outcomes.clone()).unwrap_or_default(),
            event_hashes: engine_artifacts.as_ref().map(|artifacts| artifacts.event_hashes.clone()).unwrap_or_default(),
            state_root: engine_artifacts.as_ref().and_then(|artifacts| artifacts.commits.last().map(|commit| commit.state_hash)),
        })
    }

    fn execute_node(&self, node: &SystemNode, inputs: BTreeMap<String, Value>, context: &mut ExecutionContext) -> Result<BTreeMap<String, Value>, SystemInterpreterError> {
        match node.kind {
            ...
            SystemNodeKind::Materialize => self.execute_materialize(&node.function, inputs, context),
        }
    }

    fn execute_function(&self, function_id: &FunctionId, inputs: BTreeMap<String, Value>, context: &mut ExecutionContext) -> Result<BTreeMap<String, Value>, SystemInterpreterError> {
        self.executor.execute_by_id(function_id, inputs, context).map_err(SystemInterpreterError::Executor)
    }

    fn execute_gate(&self, function_id: &FunctionId, inputs: BTreeMap<String, Value>, context: &mut ExecutionContext) -> Result<BTreeMap<String, Value>, SystemInterpreterError> {
    ...
    fn execute_persist(&self, function_id: &FunctionId, inputs: BTreeMap<String, Value>, context: &mut ExecutionContext) -> Result<BTreeMap<String, Value>, SystemInterpreterError> {
        // Persist nodes model durable writes; currently they run the bound function.
        self.execute_function(function_id, inputs, context)
    }

    fn execute_materialize(&self, function_id: &FunctionId, inputs: BTreeMap<String, Value>, context: &mut ExecutionContext) -> Result<BTreeMap<String, Value>, SystemInterpreterError> {
...
pub enum SystemInterpreterError {
    ...
    MissingField { node: SystemNodeId, output: String, field: String },
    #[error("node `{node}` output `{output}` invalid: {message}")]
    OutputTypeMismatch { node: SystemNodeId, output: String, message: String },
...
impl<'a> SystemInterpreter<'a> {
    fn commit_emitted_deltas(&self, graph_id: &str, deltas: &[crate::runtime::value::DeltaValue]) -> Result<Option<EngineArtifacts>, SystemInterpreterError> {
        if deltas.is_empty() {
            return Ok(None);
        }

        let encoded = deltas.iter().map(Self::encode_delta).collect::<Result<Vec<_>, _>>()?;

        let delta_hashes: Vec<EngineHash> = encoded.into_iter().map(|delta| self.engine.register_delta(delta)).collect();

        let judgment_proof = Self::build_judgment_proof(graph_id, deltas);
        let admission = self.engine.admit_execution(&judgment_proof).map_err(|err| SystemInterpreterError::MemoryEngine(err.into()))?;
        ...
        Ok(Some(EngineArtifacts { judgment_proof, admission, commits, outcomes, event_hashes }))
    }

    fn encode_delta(delta: &crate::runtime::value::DeltaValue) -> Result<EngineDelta, SystemInterpreterError> {
        ...
        .map_err(|source| SystemInterpreterError::DeltaEncoding { delta_id: delta.delta_id.clone(), source })
    }

    fn build_judgment_proof(graph_id: &str, deltas: &[crate::runtime::value::DeltaValue]) -> JudgmentProof {
        ...
        JudgmentProof { approved: true, timestamp: Self::current_timestamp(), hash }
    }
