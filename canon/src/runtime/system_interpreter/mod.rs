...
use thiserror::Error;
use crate::ir::{
    SystemState, FunctionId, SystemGraph, SystemNode, SystemNodeId, SystemNodeKind,
};
...
impl<'a> SystemInterpreter<'a> {
    pub fn new(ir: &'a SystemState, engine: &'a MemoryEngine) -> Self {
        Self {
            ir,
    ...
    fn run_graph(
        ...
        events
            .push(SystemExecutionEvent::Validation {
        ...
        for node_id in &order {
            ...
            let inputs = self
                .gather_inputs(node_id, &dependencies, &results, &initial_inputs);
            ...
            events
                .push(SystemExecutionEvent::NodeExecuted {
        ...
        Ok(SystemExecutionResult {
            ...
            state_root: engine_artifacts
                .as_ref()
                .and_then(|artifacts| {
                    artifacts.commits.last().map(|commit| commit.state_hash)
                }),
        })
    }
    fn execute_node(
        ...
        match node.kind {
            SystemNodeKind::Function => {
                self.execute_function(&node.function, inputs, context)
            }
            SystemNodeKind::Gate => self.execute_gate(&node.function, inputs, context),
            SystemNodeKind::Persist => {
                self.execute_persist(&node.function, inputs, context)
            }
...
pub enum SystemInterpreterError {
    ...
    DeltaEncoding { delta_id: String, #[source] source: DeltaError },
...
impl<'a> SystemInterpreter<'a> {
    fn commit_emitted_deltas(
        ...
        for commit in &commits {
            ...
            event_hashes
                .push(self.engine.compute_event_hash(&admission, commit, &outcome));
            outcomes.push(outcome);
        }
        Ok(
            Some(EngineArtifacts {
                judgment_proof,
                admission,
                commits,
                outcomes,
