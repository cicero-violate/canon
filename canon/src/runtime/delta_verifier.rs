...
use thiserror::Error;
use crate::ir::{
    AdmissionId, AppliedDeltaRecord, SystemState, StateChange, DeltaId, Proof,
};
...
impl ChangeVerifier {
    ...
    pub fn verify_application(
        original_ir: &SystemState,
        evolved_ir: &SystemState,
    ...
    fn verify_delta_proofs(
        ...
        let proofs: HashMap<_, _> = ir
            .proofs
            .iter()
            .map(|p| (p.id.as_str(), p))
            .collect();
        let deltas: HashMap<_, _> = ir
            .deltas
            .iter()
            .map(|d| (d.id.as_str(), d))
            .collect();
        ...
        for admission_id in admission_ids {
            let admission = admissions
                ...
                .ok_or_else(|| VerificationError::UnknownAdmission(
                    admission_id.clone(),
                ))?;
    ...
    fn verify_proof_scope(
        ...
        let valid = match delta.kind {
            ...
            DeltaKind::History => {
                proof.scope == ProofScope::Structure
                    || proof.scope == ProofScope::Execution
            }
        };
        if !valid {
    ...
    pub fn create_snapshot(ir: &SystemState) -> Snapshot {
        Snapshot {
    ...
    pub fn verify_snapshot(ir: &SystemState, snapshot: &Snapshot) -> bool {
...
fn compute_state_hash(ir: &SystemState) -> String {
...
pub enum VerificationError {
    ...
    #[error(
        "non-monotonic delta order: delta `{delta_id}` has order {order} <= {previous}"
    )]
