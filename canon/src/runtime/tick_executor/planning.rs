...
use crate::ir::{
    DeltaId, ExecutionRecord, Decision, JudgmentDecision, Rule, Plan, Proposal,
...
use super::types::{
    default_predicted_snapshot, PlanContext, PredictionContext, TickExecutionResult,
};
pub(super) fn plan_tick(
    ...
    if let Some((rollout, utility)) = search_best_depth(
        ir,
        tick_id,
        3,
        ...
        ir.world_model
            .push_prediction_head(PredictionHead {
...
fn search_best_depth(
    ...
    for depth in 1..=max_depth {
        if let Ok(result) = engine.rollout(tick_id, depth, inputs.clone()) {
            let utility = result.total_reward
                + world_model_bonus(ir, tick_id, result.total_reward);
...
fn world_model_bonus(ir: &SystemState, tick_id: &str, reward: f64) -> f64 {
    ir.world_model
        .prediction_head
        .iter()
        .rev()
...
pub(super) fn finalize_execution(
    ...
    println!(
        "[planner] tick={} depth={} planned={} actual={} delta={}", tick_id,
        planning_depth, planned_utility, actual, delta
    );
}
...
fn ensure_planning_judgment(ir: &mut SystemState) -> String {
