...
use crate::ir::{SystemState, ExecutionGraph};
...
pub struct ExecutionEngine<'a> {
    ir: &'a mut SystemState,
    skip_planning: bool,
}
impl<'a> ExecutionEngine<'a> {
    pub fn new(ir: &'a mut SystemState) -> Self {
        Self::new_with_skip_planning(ir, false)
    }
    pub fn new_with_skip_planning(ir: &'a mut SystemState, skip_planning: bool) -> Self {
    ...
    fn execute_graph(
        ...
        if matches!(mode, TickExecutionMode::ParallelVerified) {
            ...
            let (parallel_results, parallel_deltas) = execute_parallel(
                self.ir,
                &execution_order,
                &dependencies,
                initial_inputs,
