...
use crate::layout::{FileTopology, LayoutModule};
...
pub fn verify_dot(
    ...
    let na: BTreeMap<&str, BTreeSet<&str>> = a
        .clusters
        .iter()
        .map(|c| (c.id.as_str(), c.nodes.iter().map(|n| n.id.as_str()).collect()))
        .collect();
    ...
    for (cid, nodes_a) in &na {
        if let Some(nodes_b) = nb.get(cid) {
            for n in nodes_a.difference(nodes_b) {
                mismatches
                    .push(
                        format!("cluster `{cid}`: node `{n}` in original but not export"),
                    );
            }
            for n in nodes_b.difference(nodes_a) {
                mismatches
                    .push(
                        format!("cluster `{cid}`: node `{n}` in export but not original"),
                    );
            }
        }
    }
    ...
    for e in ea.difference(&eb) {
        mismatches
            .push(
                format!("edge `{}`->`{}` {:?} in original but not export", e.0, e.1, e.2),
            );
    }
    for e in eb.difference(&ea) {
        mismatches
            .push(
                format!("edge `{}`->`{}` {:?} in export but not original", e.0, e.1, e.2),
            );
    }
    if mismatches.is_empty() { Ok(()) } else { Err(DotVerifyError { mismatches }) }
}
const PALETTE: &[&str] = &[
    "#0055aa",
    "#884400",
...
pub fn export_dot(ir: &SystemState, layout: &FileTopology) -> String {
    ...
    for edge in &ir.module_edges {
        ...
        out.push_str(
            &format!(
                "    {} -> {} [{} color=\"{}\" ltail={} lhead={}];\n", sanitize_node_id(&
                from_node), sanitize_node_id(& to_node), if label_attr.is_empty() {
                String::new() } else { format!("{} ", label_attr.trim_start()) }, color,
                cluster_id_of(& edge.source), cluster_id_of(& edge.target),
            ),
        );
    }
    out.push_str("}\n");
