...
fn ensure_assignment(
    ...
    if layout
        .routing
        .iter()
        .any(|assignment| assignment.node == node)
    {
        return;
    }
    let module_name = ir
        ...
        .map(|m| m.name.clone())
        .unwrap_or_else(|| Word::new(module_id).unwrap_or_else(|_| Word::new("Module").unwrap()));
    ...
    layout.routing.push(LayoutAssignment {
        node,
        file_id,
