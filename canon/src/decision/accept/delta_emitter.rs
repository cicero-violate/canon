...
use crate::{
    ir::proposal::{
        derive_word_from_identifier, sanitize_identifier, ModuleSpec,
        ResolvedProposalNodes, StructSpec, TraitSpec,
    },
    ir::{
        StateChange, DeltaId, DeltaKind, ChangePayload, FunctionSignature, PipelineStage,
        Proposal, ProposedEdge, TraitFunction, TypeKind, TypeRef, ValuePort, Visibility,
        Word,
    },
};
...
pub(super) fn emit_deltas(
...
) -> Result<(Vec<StateChange>, Vec<DeltaId>), AcceptProposalError> {
    let mut deltas = Vec::new();
    let mut delta_ids = Vec::new();
    for module in sorted_modules(&resolved.modules) {
        let payload = ChangePayload::AddModule {
    ...
    for structure in sorted_structs(&resolved.structs) {
        let payload = ChangePayload::AddStruct {
    ...
    for trait_spec in sorted_traits(&resolved.traits) {
        let payload = ChangePayload::AddTrait {
    ...
    for (trait_id, functions) in trait_function_map {
        for function in functions {
            let payload = ChangePayload::AddTraitFunction {
    ...
    for structure in sorted_structs(&resolved.structs) {
        for trait_id in trait_targets.iter() {
            ...
            let impl_id = format!(
                "impl.{}.{}", sanitize_identifier(& structure.id),
                sanitize_identifier(trait_id),
            );
            let payload = ChangePayload::AddImpl {
                module: structure.module.clone(),
            ...
            if let Some(functions) = trait_function_map.get(trait_id) {
                for function in functions {
                    ...
                    let function_id = format!(
                        "fn.{}.{}", sanitize_identifier(& impl_id), sanitize_identifier(&
                        function.id),
                    );
                    let payload = ChangePayload::AddFunction {
    ...
    for edge in sorted_edges(&proposal.edges) {
        let payload = ChangePayload::AddModuleEdge {
...
fn default_trait_output(name: &Word) -> Result<ValuePort, AcceptProposalError> {
    let output_name = Word::new(format!("{}Output", name.as_str()))
        .map_err(AcceptProposalError::Word)?;
    let ty_name = Word::new(format!("{}Result", name.as_str()))
        .map_err(AcceptProposalError::Word)?;
    Ok(ValuePort {
        name: output_name,
        ty: TypeRef {
...
fn build_delta(
    ...
    artifact_label: &str,
) -> Result<StateChange, AcceptProposalError> {
    Ok(StateChange {
        ...
        description: format!(
            "Proposal `{}` impacts `{}`.", input.proposal_id, artifact_label
        ),
        related_function,
...
fn delta_identifier(proposal_id: &str, kind: &str, artifact: &str) -> String {
    format!(
        "delta.{}.{}.{}", sanitize_identifier(proposal_id), kind,
