...
use crate::{
    ...
    ir::{SystemState, Proposal, ProposalStatus, TraitFunction},
};
use super::AcceptProposalError;
pub(super) fn enforce_proposal_ready(
    proposal: &Proposal,
...
pub(super) fn enforce_references(
    ...
    let existing_modules: HashSet<&str> = ir
        .modules
        .iter()
        .map(|m| m.id.as_str())
        .collect();
    ...
    let known_modules: HashSet<&str> = existing_modules
        ...
        .collect();
    let existing_traits: HashSet<&str> = ir
        .traits
        .iter()
        .map(|t| t.id.as_str())
        .collect();
