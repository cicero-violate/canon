...
pub(crate) fn flatten_use_tree(prefix: Vec<String>, tree: &syn::UseTree, leading_colon: bool, acc: &mut Vec<UseEntry>) {
    match tree {
        syn::UseTree::Path(path) => {
            let mut next = prefix;
        ...
        syn::UseTree::Rename(rename) => {
            ...
            acc.push(UseEntry { segments, alias: Some(rename.rename.to_string()), is_glob: false, leading_colon });
        }
        syn::UseTree::Glob(_) => {
...
pub(crate) fn resolve_use_entry(entry: &UseEntry, module_key: &str) -> Option<(String, String)> {
    ...
    let mut base = if entry.leading_colon { Vec::new() } else { module_segments_from_key(module_key) };
    ...
    let imported = if let Some(alias) = &entry.alias { format!("{item_name} as {alias}") } else { item_name };
