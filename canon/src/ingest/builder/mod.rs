use std::path::Path;
use crate::layout::{LayoutMap, ParsedModel};
...
pub(crate) fn build_layout_map(
    ...
    let ModulesBuild { modules, module_lookup, file_lookup } = modules::build_modules(
        &parsed,
    )?;
    ...
    for file in &parsed.files {
        ...
        let file_id = format!(
            "file.{}", { let mut out = String::new(); for ch in path.chars() { if ch
            .is_ascii_alphanumeric() { out.push(ch.to_ascii_lowercase()); } else { out
            .push('_'); } } if out.is_empty() { "root".to_string() } else { out } }
        );
    ...
    let structs = functions::build_structs(
        &parsed,
        &module_lookup,
        &file_lookup,
        &mut layout_acc,
    );
    let enums = functions::build_enums(
        &parsed,
        &module_lookup,
        &file_lookup,
        &mut layout_acc,
    );
    let traits = functions::build_traits(
        &parsed,
        &module_lookup,
        &file_lookup,
        &mut layout_acc,
