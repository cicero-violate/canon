...
pub fn render_ast_expr(node: &JsonValue) -> String {
    ...
    match kind {
        ...
        "question" => node.get("expr").map(render_ast_expr).map(|expr| format!("{expr}?")).unwrap_or_else(|| "?".to_owned()),
...
fn render_method_expr(node: &JsonValue) -> String {
    let receiver = node.get("receiver").map(render_ast_expr).unwrap_or_else(|| "self".to_owned());
...
fn render_tuple_expr(node: &JsonValue) -> String {
    match node.get("elems").and_then(|e| e.as_array()) {
        ...
        Some(elems) => format!("({})", elems.iter().map(render_ast_expr).collect::<Vec<_>>().join(", ")),
        None => "()".to_owned(),
    }
}

fn render_array_expr(node: &JsonValue) -> String {
    let elems = node.get("elems").and_then(|e| e.as_array()).map(|arr| arr.iter().map(render_ast_expr).collect::<Vec<_>>().join(", ")).unwrap_or_default();
    format!("[{elems}]")
}
...
fn render_closure_expr(node: &JsonValue) -> String {
    ...
    let params = node.get("params").and_then(|p| p.as_array()).map(|arr| arr.iter().filter_map(|v| v.as_str().map(str::to_owned)).collect::<Vec<_>>()).unwrap_or_default();
    let params_rendered = if params.is_empty() { "||".to_owned() } else { format!("|{}|", params.join(", ")) };
    let move_prefix = if node.get("move").and_then(|m| m.as_bool()).unwrap_or(false) { "move " } else { "" };
...
fn render_args(node: &JsonValue) -> String {
    node.get("args").and_then(|a| a.as_array()).map(|arr| arr.iter().map(render_ast_expr).collect::<Vec<_>>().join(", ")).unwrap_or_default()
}
