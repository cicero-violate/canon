...
use crate::ir::{SystemState, Function, Struct, Trait, TypeKind};
use crate::layout::{LayoutFile, FileTopology, LayoutModule, LayoutNode};
...
pub fn render_file(
    ...
    let mut combined_use_lines = collect_intramodule_types(
        module,
        file_node.id.as_str(),
        layout,
        ir,
    ...
    let mut impls: Vec<_> = ir
        ...
        .filter(|block| {
            block
                ...
                .iter()
                .any(|binding| {
                    function_layout_file(layout, binding.function.as_str())
                        == Some(layout_file_id)
                })
        })
        .collect();
...
pub fn render_module_items_block(module: &crate::ir::Module) -> Option<String> {
    ...
    if !module.constants.is_empty() {
        for constant in &module.constants {
            lines
                .push(
                    format!(
                        "pub const {}: {} = {};", constant.name, render_type(& constant
                        .ty), constant.value_expr
                    ),
                );
        }
    }
    if !module.statics.is_empty() {
        for item in &module.statics {
            ...
            lines
                .push(
                    format!(
                        "{}static {mut_kw}{}: {} = {};", render_visibility(item
                        .visibility), item.name, render_type(& item.ty), item.value_expr
                    ),
                );
        }
    }
    if !module.type_aliases.is_empty() {
        for alias in &module.type_aliases {
            lines
                .push(
                    format!("pub type {} = {};", alias.name, render_type(& alias.target)),
                );
        }
    }
...
fn function_layout_file<'a>(
    layout: &'a FileTopology,
    function_id: &str,
) -> Option<&'a str> {
    layout
        .routing
        .iter()
        .find(|assign| {
            matches!(assign.node, LayoutNode::Function(ref id) if id == function_id)
        })
        .map(|assign| assign.file_id.as_str())
}
fn struct_layout_file<'a>(layout: &'a FileTopology, struct_id: &str) -> Option<&'a str> {
    layout
        .routing
        .iter()
        .find(|assign| {
            matches!(assign.node, LayoutNode::Struct(ref id) if id == struct_id)
        })
        .map(|assign| assign.file_id.as_str())
}
fn trait_layout_file<'a>(layout: &'a FileTopology, trait_id: &str) -> Option<&'a str> {
    layout
        .routing
        .iter()
        .find(|assign| {
            matches!(assign.node, LayoutNode::Trait(ref id) if id == trait_id)
        })
        .map(|assign| assign.file_id.as_str())
}
