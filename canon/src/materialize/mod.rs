...
use crate::ir::{SystemState, Function, Struct, Trait};
...
pub fn materialize(
    ...
    let struct_map: HashMap<&str, &Struct> = ir
        .structs
        .iter()
        .map(|s| (s.id.as_str(), s))
        .collect();
    let trait_map: HashMap<&str, &Trait> = ir
        .traits
        .iter()
        .map(|t| (t.id.as_str(), t))
        .collect();
    let function_map: HashMap<&str, &Function> = ir
        .functions
        .iter()
        .map(|f| (f.id.as_str(), f))
        .collect();
    ...
    for module in &modules {
        ...
        if layout_module.map(|m| m.files.is_empty()).unwrap_or(true) {
            ...
            let mut mod_sections = vec![
                "// Derived from Canonical IR. Do not edit.".to_owned()
            ];
