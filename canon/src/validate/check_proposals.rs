...
fn check_proposals(ir: &SystemState, idx: &Indexes, violations: &mut Vec<Violation>) {
    for proposal in &ir.proposals {
        if proposal.nodes.is_empty() || proposal.apis.is_empty()
            ...
            violations
                .push(
                    Violation::structured(
                        ...
                        proposal.id.clone(),
                        ViolationDetail::ProposalIncomplete {
            ...
            violations
                .push(
                    Violation::structured(
                        CanonRule::ProposalDeclarative,
                        proposal.id.clone(),
                        ViolationDetail::ProposalMissingGoal {
                            proposal: proposal.id.clone(),
                        },
                    ),
                );
                ...
                violations
                    .push(
                        Violation::structured(
                            CanonRule::ProposalDeclarative,
                            proposal.id.clone(),
                            ViolationDetail::ProposalInvalid {
                                proposal: proposal.id.clone(),
                            },
                        ),
                    );
        ...
        let proposed_mods: HashSet<&str> = resolved
            .modules
            .iter()
            .map(|m| m.id.as_str())
            .collect();
        let proposed_traits: HashSet<&str> = resolved
            .traits
            .iter()
            .map(|t| t.id.as_str())
            .collect();
                ...
                violations
                    .push(
                        Violation::structured(
                            CanonRule::ProposalDeclarative,
                            proposal.id.clone(),
                            ViolationDetail::ProposalUnknownModule {
                                proposal: proposal.id.clone(),
                ...
                violations
                    .push(
                        Violation::structured(
                            CanonRule::ProposalDeclarative,
                            proposal.id.clone(),
                            ViolationDetail::ProposalUnknownEdgeModule {
                                proposal: proposal.id.clone(),
                ...
                violations
                    .push(
                        Violation::structured(
                            CanonRule::ProposalDeclarative,
                            proposal.id.clone(),
                            ViolationDetail::ProposalUnknownTrait {
                                proposal: proposal.id.clone(),
...
fn check_judgments(ir: &SystemState, idx: &Indexes, violations: &mut Vec<Violation>) {
            ...
            violations
                .push(
                    Violation::structured(
                        ...
                        j.id.clone(),
                        ViolationDetail::JudgmentMissingProposal {
                            judgment: j.id.clone(),
                            proposal: j.proposal.clone(),
                        },
                    ),
                );
        }
            ...
            violations
                .push(
                    Violation::structured(
                        CanonRule::JudgmentDecisions,
                        j.id.clone(),
                        ViolationDetail::JudgmentMissingPredicate {
                            judgment: j.id.clone(),
...
fn check_learning(ir: &SystemState, idx: &Indexes, violations: &mut Vec<Violation>) {
            ...
            violations
                .push(
                    Violation::structured(
                        ...
                        item.id.clone(),
                        ViolationDetail::LearningMissingProposal {
                            learning: item.id.clone(),
            ...
            violations
                .push(
                    Violation::structured(
                        CanonRule::LearningDeclarations,
                        item.id.clone(),
                        ViolationDetail::LearningMissingRules {
                            learning: item.id.clone(),
                        },
                    ),
                );
            ...
            violations
                .push(
                    Violation::structured(
                        CanonRule::LearningDeclarations,
                        item.id.clone(),
                        ViolationDetail::LearningMissingProofObject {
                            learning: item.id.clone(),
                        },
                    ),
                );
...
fn check_goal_mutations(
    ir: &SystemState,
    idx: &Indexes,
    violations: &mut Vec<Violation>,
) {
    for mutation in &ir.goal_mutations {
        if mutation.status == GoalMutationStatus::Accepted
            && mutation.judgment_id.is_none()
        {
            violations
                .push(
                    Violation::structured(
                        ...
                        mutation.id.clone(),
                        ViolationDetail::GoalMutationMissingJudgment {
                ...
                violations
                    .push(
                        Violation::structured(
                            ...
                            mutation.id.clone(),
                            ViolationDetail::GoalMutationMissingProof {
                                mutation: mutation.id.clone(),
