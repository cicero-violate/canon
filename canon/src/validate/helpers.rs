...
pub struct Indexes<'a> {
    ...
    pub deltas:      HashMap<&'a str, &'a Delta>,
    pub proofs:      HashMap<&'a str, &'a Proof>,
    pub predicates:  HashMap<&'a str, &'a JudgmentPredicate>,
    pub judgments:   HashMap<&'a str, &'a Judgment>,
    pub admissions:  HashMap<&'a str, &'a DeltaAdmission>,
    pub tick_graphs: HashMap<&'a str, &'a TickGraph>,
    pub ticks:       HashMap<&'a str, &'a Tick>,
    pub epochs:      HashMap<&'a str, &'a TickEpoch>,
...
pub fn build_indexes<'a>(
    ...
    Indexes {
        ...
        impls:       index_by_id(&ir.impl_blocks,        |i| i.id.as_str(), CanonRule::ImplBinding,        "impl",             violations),
    ...
    F: Fn(&'a T) -> &'a str,
{
    ...
    for item in items {
        ...
        if map.insert(id, item).is_some() {
            violations.push(Violation::new(rule, format!("duplicate {kind} id `{id}` is forbidden")));
        }
    }
    map
}
...
pub fn module_has_permission<'a>(
    ...
    while let Some(node) = stack.pop() {
        ...
        if let Some(neighbors) = adjacency.get(node) {
            for nb in neighbors { stack.push(nb); }
        }
    }
    false
}
