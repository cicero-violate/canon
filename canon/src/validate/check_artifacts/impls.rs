...
pub fn check_impls(ir: &SystemState, idx: &Indexes, violations: &mut Vec<Violation>) {
    let enum_ids: std::collections::HashSet<&str> = ir
        .enums
        .iter()
        .map(|e| e.id.as_str())
        .collect();
    ...
    for block in &ir.impls {
        ...
        let struct_id_valid = struct_opt.is_some()
            || enum_ids.contains(block.struct_id.as_str());
        if !struct_id_valid {
            violations
                .push(
                    Violation::structured(
                        ...
                        block.id.clone(),
                        ViolationDetail::ImplMissingStruct {
            ...
            violations
                .push(
                    Violation::structured(
                        CanonRule::ImplBinding,
                        block.id.clone(),
                        ViolationDetail::ImplMissingTrait {
                            impl_id: block.id.clone(),
            ...
            violations
                .push(
                    Violation::structured(
                        CanonRule::ImplBinding,
                        block.id.clone(),
                        ViolationDetail::ImplMissingModule {
                            impl_id: block.id.clone(),
                    ...
                    violations
                        .push(
                            Violation::structured(
                                CanonRule::ImplBinding,
                                block.id.clone(),
                                ViolationDetail::ImplWrongModuleForStruct {
                                    impl_id: block.id.clone(),
                                },
                            ),
                        );
                ...
                violations
                    .push(
                        Violation::structured(
                            ...
                            block.id.clone(),
                            ViolationDetail::ImplFunctionDuplicate {
                                function_id: binding.function.clone(),
                            },
                        ),
                    );
            }
                    ...
                    violations
                        .push(
                            Violation::structured(
                                CanonRule::ImplBinding,
                                block.id.clone(),
                                ViolationDetail::ImplTraitMismatch {
                                    impl_id: block.id.clone(),
                                    trait_fn: binding.trait_fn.clone(),
                                },
                            ),
                        );
                }
