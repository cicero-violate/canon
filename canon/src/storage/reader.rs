...
use crate::ir::{
    AppliedDeltaRecord, CallEdge, CanonicalIr, CanonicalMeta, Delta as CanonicalDelta, DeltaAdmission, EnumNode, ErrorArtifact, ExecutionRecord, ExternalDependency, Function, GoalMutation,
    GpuFunction, ImplBlock, Judgment, JudgmentPredicate, Learning, LoopPolicy, Module, ModuleEdge, Plan, PolicyParameters, Project, Proof, Proposal, RewardRecord, Struct, SystemGraph, Tick,
    TickEpoch, TickGraph, Trait, VersionContract,
};
...
impl<'a> MemoryIrReader<'a> {
    pub fn new(state: &'a MerkleState) -> Self {
        Self { state, layout: MemoryArtifactLayout::new() }
    }
    pub fn read_ir(&self) -> Result<CanonicalIr, MemoryIrReadError> {
        let manifest: ArtifactManifest = self.read_entry_in_slot(ArtifactSegment::Meta, "manifest", META_MANIFEST_SLOT)?;
        let meta: CanonicalMeta = self.read_entry_in_slot(ArtifactSegment::Meta, "meta", META_STATE_SLOT)?;
        let version_contract: VersionContract = self.read_singleton(ArtifactSegment::VersionContract, "version_contract")?;
        let project: Project = self.read_singleton(ArtifactSegment::Project, "project")?;
        let world_model: WorldModel = self.read_singleton(ArtifactSegment::WorldModel, "world_model")?;
        ...
        let module_edges = self.read_collection(ArtifactSegment::ModuleEdge, &manifest.module_edges)?;
        ...
        let tick_graphs = self.read_collection(ArtifactSegment::TickGraph, &manifest.tick_graphs)?;
        let system_graphs = self.read_collection(ArtifactSegment::SystemGraph, &manifest.system_graphs)?;
        let loop_policies = self.read_collection(ArtifactSegment::LoopPolicy, &manifest.loop_policies)?;
        let tick_epochs = self.read_collection(ArtifactSegment::TickEpoch, &manifest.tick_epochs)?;
        let policy_parameters = self.read_collection(ArtifactSegment::Policy, &manifest.policies)?;
        ...
        let applied_deltas = self.read_collection(ArtifactSegment::AppliedDelta, &manifest.applied_deltas)?;
        let gpu_functions = self.read_collection(ArtifactSegment::GpuFunction, &manifest.gpu_functions)?;
        ...
        let predicates = self.read_collection(ArtifactSegment::JudgmentPredicate, &manifest.judgment_predicates)?;
        ...
        let dependencies = self.read_collection(ArtifactSegment::Dependency, &manifest.dependencies)?;
        let file_hash_records: Vec<FileHashRecord> = self.read_collection(ArtifactSegment::FileHash, &manifest.file_hashes)?;
        ...
        let goal_mutations = self.read_collection(ArtifactSegment::GoalMutation, &manifest.goal_mutations)?;
        Ok(CanonicalIr {
            meta,
            version_contract,
            project,
    ...
    pub fn read_ir_from_checkpoint<P: AsRef<Path>>(path: P) -> Result<CanonicalIr, MemoryIrReadError> {
    ...
    fn read_collection<T: DeserializeOwned>(&self, segment: ArtifactSegment, entries: &[ManifestEntry]) -> Result<Vec<T>, MemoryIrReadError> {
    ...
    fn read_singleton<T: DeserializeOwned>(&self, segment: ArtifactSegment, artifact_id: &str) -> Result<T, MemoryIrReadError> {
        self.read_entry_in_slot(segment, artifact_id, SINGLETON_SLOT)
    }
    fn read_entry_in_slot<T: DeserializeOwned>(&self, segment: ArtifactSegment, artifact_id: &str, slot: u64) -> Result<T, MemoryIrReadError> {
        let bytes = self.read_bytes(segment, artifact_id, slot)?;
        DefaultOptions::new().with_fixint_encoding().allow_trailing_bytes().deserialize(&bytes).map_err(MemoryIrReadError::Decode)
    }
    fn read_bytes(&self, segment: ArtifactSegment, artifact_id: &str, slot: u64) -> Result<Vec<u8>, MemoryIrReadError> {
        ...
        if first_page.len() < LENGTH_PREFIX {
            return Err(MemoryIrReadError::Corrupt { segment, artifact_id: artifact_id.to_owned(), reason: "page shorter than prefix".to_owned() });
        }
