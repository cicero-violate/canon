use crate::ir::{
    AppliedDeltaRecord, CallEdge, CanonicalIr, Delta as CanonicalDelta, DeltaAdmission, EnumNode, ErrorArtifact, ExecutionRecord, ExternalDependency, Function, GoalMutation, GpuFunction, ImplBlock,
    Judgment, JudgmentPredicate, Learning, LoopPolicy, Module, ModuleEdge, Plan, PolicyParameters, Proof, Proposal, RewardRecord, Struct, SystemGraph, Tick, TickEpoch, TickGraph, Trait,
};
use crate::storage::layout::{ArtifactSegment, LayoutError, MemoryArtifactLayout, META_MANIFEST_SLOT, META_STATE_SLOT, PAGE_BYTES, SINGLETON_SLOT};
...
impl<'engine> MemoryIrBuilder<'engine> {
    pub fn new(engine: &'engine MemoryEngine) -> Self {
        Self { engine, layout: MemoryArtifactLayout::new(), judgment_counter: AtomicU64::new(0) }
    }
    ...
    pub fn write_ir_to_disk(&self, ir: &CanonicalIr) -> Result<(), MemoryIrBuilderError> {
        ...
        self.write_artifact_page(ArtifactSegment::Meta, "manifest", META_MANIFEST_SLOT, &manifest)?;
        self.write_artifact_page(ArtifactSegment::Meta, "meta", META_STATE_SLOT, &ir.meta)?;
        self.write_artifact_page(ArtifactSegment::VersionContract, "version_contract", SINGLETON_SLOT, &ir.version_contract)?;
        self.write_artifact_page(ArtifactSegment::Project, "project", SINGLETON_SLOT, &ir.project)?;
        self.write_artifact_page(ArtifactSegment::WorldModel, "world_model", SINGLETON_SLOT, &ir.world_model)?;
        ...
        for function in &ir.functions {
            let slot = slot_lookup.require_slot(ArtifactSegment::Function, &function.id)?;
        ...
        for loop_policy in &ir.loop_policies {
            let slot = slot_lookup.require_slot(ArtifactSegment::LoopPolicy, &loop_policy.id)?;
            self.write_artifact_page(ArtifactSegment::LoopPolicy, &loop_policy.id, slot, loop_policy)?;
        }
        ...
        for tick_graph in &ir.tick_graphs {
            let slot = slot_lookup.require_slot(ArtifactSegment::TickGraph, &tick_graph.id)?;
            self.persist_tick_graph(&tick_graph.id, slot, tick_graph)?;
        }
        for system in &ir.system_graphs {
            let slot = slot_lookup.require_slot(ArtifactSegment::SystemGraph, &system.id)?;
        ...
        for epoch in &ir.tick_epochs {
            ...
            self.write_artifact_page(ArtifactSegment::TickEpoch, &epoch.id, slot, epoch)?;
        }
        ...
        for execution in &ir.executions {
            let slot = slot_lookup.require_slot(ArtifactSegment::Execution, &execution.id)?;
            self.write_artifact_page(ArtifactSegment::Execution, &execution.id, slot, execution)?;
        }
        for admission in &ir.admissions {
            let slot = slot_lookup.require_slot(ArtifactSegment::Admission, &admission.id)?;
            self.write_artifact_page(ArtifactSegment::Admission, &admission.id, slot, admission)?;
        }
        for applied in &ir.applied_deltas {
            let slot = slot_lookup.require_slot(ArtifactSegment::AppliedDelta, &applied.id)?;
            self.write_artifact_page(ArtifactSegment::AppliedDelta, &applied.id, slot, applied)?;
        }
        for proposal in &ir.proposals {
            let slot = slot_lookup.require_slot(ArtifactSegment::Proposal, &proposal.id)?;
            self.write_artifact_page(ArtifactSegment::Proposal, &proposal.id, slot, proposal)?;
        }
        for judgment in &ir.judgments {
            let slot = slot_lookup.require_slot(ArtifactSegment::Judgment, &judgment.id)?;
            self.write_artifact_page(ArtifactSegment::Judgment, &judgment.id, slot, judgment)?;
        }
        for predicate in &ir.judgment_predicates {
            let slot = slot_lookup.require_slot(ArtifactSegment::JudgmentPredicate, &predicate.id)?;
            self.write_artifact_page(ArtifactSegment::JudgmentPredicate, &predicate.id, slot, predicate)?;
        }
        ...
        for err in &ir.errors {
            let slot = slot_lookup.require_slot(ArtifactSegment::ErrorArtifact, &err.id)?;
            self.write_artifact_page(ArtifactSegment::ErrorArtifact, &err.id, slot, err)?;
        }
        ...
        for (path, hash) in &ir.file_hashes {
            ...
            let record = FileHashRecord { path: path.clone(), hash: hash.clone() };
        ...
        for goal in &ir.goal_mutations {
            let slot = slot_lookup.require_slot(ArtifactSegment::GoalMutation, &goal.id)?;
            self.write_artifact_page(ArtifactSegment::GoalMutation, &goal.id, slot, goal)?;
        }
        Ok(())
    }
    pub fn persist_module(&self, module: &Module, slot: u64) -> Result<(), MemoryIrBuilderError> {
        self.write_artifact_page(ArtifactSegment::Module, &module.id, slot, module)
    }
    pub fn persist_struct(&self, structure: &Struct, slot: u64) -> Result<(), MemoryIrBuilderError> {
        self.write_artifact_page(ArtifactSegment::Struct, &structure.id, slot, structure)
    }
    pub fn persist_enum(&self, enum_node: &EnumNode, slot: u64) -> Result<(), MemoryIrBuilderError> {
        self.write_artifact_page(ArtifactSegment::Enum, &enum_node.id, slot, enum_node)
    }
    pub fn persist_trait(&self, tr: &Trait, slot: u64) -> Result<(), MemoryIrBuilderError> {
        self.write_artifact_page(ArtifactSegment::Trait, &tr.id, slot, tr)
    }
    pub fn persist_impl(&self, block: &ImplBlock, slot: u64) -> Result<(), MemoryIrBuilderError> {
        self.write_artifact_page(ArtifactSegment::ImplBlock, &block.id, slot, block)
    }
    pub fn persist_function(&self, function: &Function, slot: u64) -> Result<(), MemoryIrBuilderError> {
        self.write_artifact_page(ArtifactSegment::Function, &function.id, slot, function)
    }
    pub fn persist_module_edge(&self, edge: &ModuleEdge, slot: u64) -> Result<(), MemoryIrBuilderError> {
        let id = format!("edge.module.{}->{}", edge.source, edge.target);
        self.write_artifact_page(ArtifactSegment::ModuleEdge, &id, slot, edge)
    }
    pub fn persist_call_edge(&self, edge: &CallEdge, slot: u64) -> Result<(), MemoryIrBuilderError> {
        let id = format!("edge.call.{}->{}", edge.caller, edge.callee);
        self.write_artifact_page(ArtifactSegment::CallEdge, &id, slot, edge)
    }
    pub fn persist_tick_graph(&self, tick_id: &str, slot: u64, graph: &TickGraph) -> Result<(), MemoryIrBuilderError> {
        self.write_artifact_page(ArtifactSegment::TickGraph, tick_id, slot, graph)
    }
    pub fn persist_system_graph(&self, system_id: &str, slot: u64, graph: &SystemGraph) -> Result<(), MemoryIrBuilderError> {
        self.write_artifact_page(ArtifactSegment::SystemGraph, system_id, slot, graph)
    }
    pub fn persist_reward<T: Serialize>(&self, key: &str, slot: u64, value: &T) -> Result<(), MemoryIrBuilderError> {
        self.write_artifact_page(ArtifactSegment::Reward, key, slot, value)
    }
    fn write_artifact_page<T: Serialize>(&self, segment: ArtifactSegment, artifact_id: &str, slot: u64, value: &T) -> Result<(), MemoryIrBuilderError> {
        ...
        if chunk_count > crate::storage::layout::PAGES_PER_SLOT {
            return Err(MemoryIrBuilderError::Layout(LayoutError::ChunkOverflow {
                segment,
        ...
        for chunk_index in 0..chunk_count {
            ...
            let delta = Delta::new_dense(DeltaID(address.page_id), PageID(address.page_id), Epoch(0), chunk.to_vec(), mask, Source(format!("canon.artifact::{:?}::{artifact_id}", segment)))
                .map_err(|err| MemoryIrBuilderError::Delta { artifact_id: artifact_id.to_owned(), source: err })?;
            deltas.push(delta);
        }
    ...
    fn encode<T: Serialize>(&self, value: &T) -> Result<Vec<u8>, MemoryIrBuilderError> {
        DefaultOptions::new().with_fixint_encoding().allow_trailing_bytes().serialize(value).map_err(MemoryIrBuilderError::Serialize)
    }
    fn admit_and_commit_pages(&self, segment: ArtifactSegment, artifact_id: &str, deltas: Vec<Delta>) -> Result<(), MemoryIrBuilderError> {
        ...
        self.engine.commit_batch(&admission, &hashes).map_err(MemoryIrBuilderError::Memory)?;
        Ok(())
    }
    fn build_builder_judgment_proof(&self, segment: ArtifactSegment, artifact_id: &str, hashes: &[EngineHash]) -> JudgmentProof {
        ...
        JudgmentProof { approved: true, timestamp, hash: bytes }
    }
}
#[derive(Debug, Error)]
pub enum MemoryIrBuilderError {
    ...
    Delta {
        artifact_id: String,
        #[source]
        source: database::delta::DeltaError,
    },
...
impl ManifestSlotLookup {
    fn new(manifest: &ArtifactManifest) -> Self {
        ...
        Self::index_segment(&mut slots, ArtifactSegment::ModuleEdge, &manifest.module_edges);
        ...
        Self::index_segment(&mut slots, ArtifactSegment::TickGraph, &manifest.tick_graphs);
        Self::index_segment(&mut slots, ArtifactSegment::SystemGraph, &manifest.system_graphs);
        Self::index_segment(&mut slots, ArtifactSegment::LoopPolicy, &manifest.loop_policies);
        Self::index_segment(&mut slots, ArtifactSegment::TickEpoch, &manifest.tick_epochs);
        ...
        Self::index_segment(&mut slots, ArtifactSegment::Execution, &manifest.executions);
        Self::index_segment(&mut slots, ArtifactSegment::Admission, &manifest.admissions);
        Self::index_segment(&mut slots, ArtifactSegment::AppliedDelta, &manifest.applied_deltas);
        Self::index_segment(&mut slots, ArtifactSegment::GpuFunction, &manifest.gpu_functions);
        ...
        Self::index_segment(&mut slots, ArtifactSegment::JudgmentPredicate, &manifest.judgment_predicates);
        ...
        Self::index_segment(&mut slots, ArtifactSegment::ErrorArtifact, &manifest.errors);
        Self::index_segment(&mut slots, ArtifactSegment::Dependency, &manifest.dependencies);
        Self::index_segment(&mut slots, ArtifactSegment::FileHash, &manifest.file_hashes);
        Self::index_segment(&mut slots, ArtifactSegment::Reward, &manifest.rewards);
        Self::index_segment(&mut slots, ArtifactSegment::GoalMutation, &manifest.goal_mutations);
        Self { slots }
    }
    fn index_segment(slots: &mut HashMap<ArtifactSegment, HashMap<String, u64>>, segment: ArtifactSegment, entries: &[ManifestEntry]) {
        if entries.is_empty() {
            return;
        }
    ...
    fn require_slot(&self, segment: ArtifactSegment, artifact_id: &str) -> Result<u64, MemoryIrBuilderError> {
        self.slots.get(&segment).and_then(|entries| entries.get(artifact_id)).copied().ok_or_else(|| MemoryIrBuilderError::MissingSlot { segment, artifact_id: artifact_id.to_owned() })
    }
}
