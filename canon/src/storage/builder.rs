...
use memory_engine::{AdmissionError, AdmissionProof, CommitProof, JudgmentProof, MemoryEngine};
use serde::Serialize;
...
use crate::ir::{
    AppliedDeltaRecord, CallEdge, CanonicalIr, Delta as CanonicalDelta, DeltaAdmission, EnumNode,
    ErrorArtifact, ExecutionRecord, ExternalDependency, Function, GoalMutation, GpuFunction,
    ImplBlock, Judgment, JudgmentPredicate, Learning, LoopPolicy, Module, ModuleEdge, Plan,
    PolicyParameters, Proof, Proposal, RewardRecord, Struct, SystemGraph, Tick, TickEpoch,
    TickGraph, Trait,
};
...
impl<'engine> MemoryIrBuilder<'engine> {
    ...
    pub fn persist_ir(&self, ir: &CanonicalIr) -> Result<(), MemoryIrBuilderError> {
        ...
        self.persist_segment(ArtifactSegment::Meta, "meta", META_STATE_SLOT, &ir.meta)?;
        self.persist_segment(
        ...
        for module in &ir.modules {
            let slot = slot_lookup.slot_or_err(ArtifactSegment::Module, &module.id)?;
            self.persist_module(module, slot)?;
        }
        for structure in &ir.structs {
            let slot = slot_lookup.slot_or_err(ArtifactSegment::Struct, &structure.id)?;
            self.persist_struct(structure, slot)?;
        }
        for enum_node in &ir.enums {
            let slot = slot_lookup.slot_or_err(ArtifactSegment::Enum, &enum_node.id)?;
            self.persist_enum(enum_node, slot)?;
        }
        for tr in &ir.traits {
            let slot = slot_lookup.slot_or_err(ArtifactSegment::Trait, &tr.id)?;
            self.persist_trait(tr, slot)?;
        }
        for block in &ir.impls {
            let slot = slot_lookup.slot_or_err(ArtifactSegment::ImplBlock, &block.id)?;
            self.persist_impl(block, slot)?;
        }
        for function in &ir.functions {
            let slot = slot_lookup.slot_or_err(ArtifactSegment::Function, &function.id)?;
            self.persist_function(function, slot)?;
        }
        for gpu in &ir.gpu_functions {
            let slot = slot_lookup.slot_or_err(ArtifactSegment::GpuFunction, &gpu.id)?;
            self.persist_segment(ArtifactSegment::GpuFunction, &gpu.id, slot, gpu)?;
        }
        for edge in &ir.module_edges {
            let id = format!("edge.module.{}->{}", edge.source, edge.target);
            let slot = slot_lookup.slot_or_err(ArtifactSegment::ModuleEdge, &id)?;
            self.persist_module_edge(edge, slot)?;
        }
        for edge in &ir.call_edges {
            let id = format!("edge.call.{}->{}", edge.caller, edge.callee);
            let slot = slot_lookup.slot_or_err(ArtifactSegment::CallEdge, &id)?;
            self.persist_call_edge(edge, slot)?;
        }
        for loop_policy in &ir.loop_policies {
            let slot = slot_lookup.slot_or_err(ArtifactSegment::LoopPolicy, &loop_policy.id)?;
            self.persist_segment(
        ...
        for tick in &ir.ticks {
            let slot = slot_lookup.slot_or_err(ArtifactSegment::Tick, &tick.id)?;
            self.persist_segment(ArtifactSegment::Tick, &tick.id, slot, tick)?;
        }
        for tick_graph in &ir.tick_graphs {
            let slot = slot_lookup.slot_or_err(ArtifactSegment::TickGraph, &tick_graph.id)?;
            self.persist_tick_graph(&tick_graph.id, slot, tick_graph)?;
        }
        for system in &ir.system_graphs {
            let slot = slot_lookup.slot_or_err(ArtifactSegment::SystemGraph, &system.id)?;
            self.persist_system_graph(&system.id, slot, system)?;
        }
        for epoch in &ir.tick_epochs {
            let slot = slot_lookup.slot_or_err(ArtifactSegment::TickEpoch, &epoch.id)?;
            self.persist_segment(ArtifactSegment::TickEpoch, &epoch.id, slot, epoch)?;
        }
        for policy in &ir.policy_parameters {
            let slot = slot_lookup.slot_or_err(ArtifactSegment::Policy, &policy.id)?;
            self.persist_segment(ArtifactSegment::Policy, &policy.id, slot, policy)?;
        }
        for plan in &ir.plans {
            let slot = slot_lookup.slot_or_err(ArtifactSegment::Plan, &plan.id)?;
            self.persist_segment(ArtifactSegment::Plan, &plan.id, slot, plan)?;
        }
        for execution in &ir.executions {
            let slot = slot_lookup.slot_or_err(ArtifactSegment::Execution, &execution.id)?;
            self.persist_segment(ArtifactSegment::Execution, &execution.id, slot, execution)?;
        }
        for admission in &ir.admissions {
            let slot = slot_lookup.slot_or_err(ArtifactSegment::Admission, &admission.id)?;
            self.persist_segment(ArtifactSegment::Admission, &admission.id, slot, admission)?;
        }
        for applied in &ir.applied_deltas {
            let slot = slot_lookup.slot_or_err(ArtifactSegment::AppliedDelta, &applied.id)?;
            self.persist_segment(ArtifactSegment::AppliedDelta, &applied.id, slot, applied)?;
        }
        for proposal in &ir.proposals {
            let slot = slot_lookup.slot_or_err(ArtifactSegment::Proposal, &proposal.id)?;
            self.persist_segment(ArtifactSegment::Proposal, &proposal.id, slot, proposal)?;
        }
        for judgment in &ir.judgments {
            let slot = slot_lookup.slot_or_err(ArtifactSegment::Judgment, &judgment.id)?;
            self.persist_segment(ArtifactSegment::Judgment, &judgment.id, slot, judgment)?;
        }
        for predicate in &ir.judgment_predicates {
            let slot =
                slot_lookup.slot_or_err(ArtifactSegment::JudgmentPredicate, &predicate.id)?;
            self.persist_segment(
        ...
        for delta in &ir.deltas {
            let slot = slot_lookup.slot_or_err(ArtifactSegment::DeltaDef, &delta.id)?;
            self.persist_segment(ArtifactSegment::DeltaDef, &delta.id, slot, delta)?;
        }
        for proof in &ir.proofs {
            let slot = slot_lookup.slot_or_err(ArtifactSegment::Proof, &proof.id)?;
            self.persist_segment(ArtifactSegment::Proof, &proof.id, slot, proof)?;
        }
        for learn in &ir.learning {
            let slot = slot_lookup.slot_or_err(ArtifactSegment::Learning, &learn.id)?;
            self.persist_segment(ArtifactSegment::Learning, &learn.id, slot, learn)?;
        }
        for err in &ir.errors {
            let slot = slot_lookup.slot_or_err(ArtifactSegment::ErrorArtifact, &err.id)?;
            self.persist_segment(ArtifactSegment::ErrorArtifact, &err.id, slot, err)?;
        }
        for dep in &ir.dependencies {
            let key = format!("dependency::{}", dep.name);
            let slot = slot_lookup.slot_or_err(ArtifactSegment::Dependency, &key)?;
            self.persist_segment(ArtifactSegment::Dependency, &key, slot, dep)?;
        }
        for (path, hash) in &ir.file_hashes {
            let key = format!("filehash::{path}");
            let slot = slot_lookup.slot_or_err(ArtifactSegment::FileHash, &key)?;
            let record = FileHashRecord {
                path: path.clone(),
                hash: hash.clone(),
            };
            self.persist_segment(ArtifactSegment::FileHash, &key, slot, &record)?;
        }
        for reward in &ir.reward_deltas {
            let slot = slot_lookup.slot_or_err(ArtifactSegment::Reward, &reward.id)?;
            self.persist_reward(&reward.id, slot, reward)?;
        }
        for goal in &ir.goal_mutations {
            let slot = slot_lookup.slot_or_err(ArtifactSegment::GoalMutation, &goal.id)?;
            self.persist_segment(ArtifactSegment::GoalMutation, &goal.id, slot, goal)?;
        }
    ...
    pub fn persist_module(&self, module: &Module, slot: u64) -> Result<(), MemoryIrBuilderError> {
        self.persist_segment(ArtifactSegment::Module, &module.id, slot, module)
    }
    ...
    pub fn persist_struct(
        ...
        self.persist_segment(ArtifactSegment::Struct, &structure.id, slot, structure)
    }
    ...
    pub fn persist_enum(
        ...
        self.persist_segment(ArtifactSegment::Enum, &enum_node.id, slot, enum_node)
    }

    pub fn persist_trait(&self, tr: &Trait, slot: u64) -> Result<(), MemoryIrBuilderError> {
        self.persist_segment(ArtifactSegment::Trait, &tr.id, slot, tr)
    }

    pub fn persist_impl(&self, block: &ImplBlock, slot: u64) -> Result<(), MemoryIrBuilderError> {
        self.persist_segment(ArtifactSegment::ImplBlock, &block.id, slot, block)
    }
    ...
    pub fn persist_function(
        ...
        self.persist_segment(ArtifactSegment::Function, &function.id, slot, function)
    }
    ...
    pub fn persist_module_edge(
        ...
        let id = format!("edge.module.{}->{}", edge.source, edge.target);
        self.persist_segment(ArtifactSegment::ModuleEdge, &id, slot, edge)
    }
    ...
    pub fn persist_call_edge(
        ...
        let id = format!("edge.call.{}->{}", edge.caller, edge.callee);
        self.persist_segment(ArtifactSegment::CallEdge, &id, slot, edge)
    }
    ...
    pub fn persist_tick_graph(
        ...
        self.persist_segment(ArtifactSegment::TickGraph, tick_id, slot, graph)
    }
    ...
    pub fn persist_system_graph(
        ...
        self.persist_segment(ArtifactSegment::SystemGraph, system_id, slot, graph)
    }
    ...
    pub fn persist_reward<T: Serialize>(
        ...
        self.persist_segment(ArtifactSegment::Reward, key, slot, value)
    }

    fn persist_segment<T: Serialize>(
        &self,
        ...
        self.commit_deltas(segment, artifact_id, deltas)
    }
    ...
    fn commit_deltas(
        ...
        let proof = self.synthetic_judgment(segment, artifact_id, &hashes);
...
pub enum MemoryIrBuilderError {
    ...
    Delta {
        artifact_id: String,
        #[source]
        source: memory_engine::delta::DeltaError,
    },
