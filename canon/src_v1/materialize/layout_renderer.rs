...
pub struct LayoutLookup<'a> {
    ...
    routing: HashMap<&'a str, &'a LayoutAssignment>,
    modules: HashMap<&'a str, &'a LayoutModule>,
}

impl<'a> LayoutLookup<'a> {
    pub fn new(graph: &'a LayoutGraph) -> Self {
    ...
    pub fn assignment_for_struct(&self, structure: &Struct) -> Option<&LayoutAssignment> {
        self.routing.get(structure.id.as_str()).copied()
    }

    pub fn assignment_for_trait(&self, tr: &Trait) -> Option<&LayoutAssignment> {
