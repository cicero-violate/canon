...
impl<'a> SystemInterpreter<'a> {
    ...
    pub(super) fn validate_nodes(
        ...
        for node in node_index.values() {
            let function = function_index.get(&node.function).ok_or_else(|| {
                SystemInterpreterError::UnknownFunction {
                    node: node.id.clone(),
                    function: node.function.clone(),
                }
            })?;
            if !function.contract.total {
                return Err(SystemInterpreterError::NonTotalFunction(
                    function.id.clone(),
                ));
            }
        }
        events.push(super::SystemExecutionEvent::Validation {
