use canon::ir::{CanonicalIr, Delta, DeltaKind, Function, FunctionContract, FunctionMetadata, PipelineStage, Postcondition, TypeKind, TypeRef, ValuePort, Visibility, Word};
use canon::proof::smt_bridge::{attach_function_proofs, verify_function_postconditions, SmtError};
...
fn load_fixture(name: &str) -> CanonicalIr {
    let path = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("tests").join("data").join(name);
...
fn sample_function(expr: Expr) -> Function {
    ...
    let ast = FunctionAst { outputs: vec![OutputExpr { name: output_name.as_str().to_string(), expr }] };
    ...
    Function {
        ...
        inputs: vec![ValuePort {
            ...
            ty: TypeRef { name: Word::new("ScalarValue").unwrap(), kind: TypeKind::Scalar, params: Vec::new(), ref_kind: Default::default(), lifetime: None },
        }],
        ...
        metadata: FunctionMetadata { bytecode_b64: None, ast: Some(serde_json::to_value(&ast).unwrap()), postconditions: vec![Postcondition::NonNegative { output: output_name }] },
    }
}

#[test]
fn smt_proves_square_is_non_negative() {
    let expr = Expr::BinOp { left: Box::new(Expr::Input { name: "Input".into() }), op: BinOp::Mul, right: Box::new(Expr::Input { name: "Input".into() }) };
    let function = sample_function(expr);
    let cert = verify_function_postconditions(&function).expect("verification succeeds").expect("certificate expected");
    assert!(!cert.proof_hash.is_empty());
}

#[test]
fn smt_detects_counterexample_when_condition_fails() {
    let expr = Expr::Input { name: "Input".into() };
    let function = sample_function(expr);
...
fn attach_function_proofs_sets_delta_hash() {
    ...
    assert!(ir.deltas.last().unwrap().proof_object_hash.as_ref().is_some(), "delta must include proof hash");
}
