...
impl KernelGraphBuilder {
    ...
    pub fn add_node(&mut self, payload: NodePayload) -> Result<NodeId, GraphDeltaError> {
        ...
        let node = NodeRecord { id: node_id, key: Arc::<str>::from(payload.key), label: Arc::<str>::from(payload.label), metadata: payload.metadata };
    ...
    pub fn add_edge(&mut self, payload: EdgePayload) -> Result<EdgeId, GraphDeltaError> {
        let edge_id = EdgeId::from_components(&payload.from, &payload.to, payload.kind.as_str());
        let edge = EdgeRecord { id: edge_id, from: payload.from, to: payload.to, kind: payload.kind, metadata: payload.metadata };
        self.materializer.apply(GraphDelta::AddEdge(edge))?;
        Ok(edge_id)
    }

    pub fn merge_node_metadata<I>(&mut self, node_id: NodeId, updates: I) -> Result<(), GraphDeltaError>
