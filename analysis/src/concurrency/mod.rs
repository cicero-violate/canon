...
//!
//! Variables:
//!   lockset(t, p) = set of locks held by thread t at program point p
//!
//! Equations (Eraser-style):
//!   lockset(t, p) = ∩{ lockset(t, q) | q ∈ pred(p) }
//!   race(v) = ∃ t1 ≠ t2: v accessed by both ∧ lockset(t1) ∩ lockset(t2) = ∅
...
impl LocksetState {
    ...
    pub fn races(&self) -> Vec<VarId> {
        self.accesses.iter()
            .filter(|(_, records)| {
                // Any two accesses from different threads with empty lockset intersection
