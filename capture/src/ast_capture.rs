//! AST-based capture (stable)
//! Source₀ -> Model₀ using syn only
...
pub fn capture_ast(root: &Path) -> Result<Model> {
    ...
    for file in collect_rs_files(root)? {
        ...
        for item in ast.items {
            match item {
                ...
                syn::Item::Impl(i) => {
                    impls.push(Impl {
                        id: format!("impl:{}", order),
                        struct_: "unknown".into(),
                        trait_: None,
                        module: module_id.clone(),
                        kind: "inherent".into(),
                        order,
    ...
    Ok(Model {
        ...
        impls,
        functions,
        // --- HIR unified layer (AST-only capture => empty) ---
        hir_items: Vec::new(),
        hir_exprs: Vec::new(),
        hir_paths: Vec::new(),
        hir_types: Vec::new(),
        mir_bodies: Vec::new(),
        rules: Rules {
            pairing_scope: "module".into(),
            impl_placement: "same_module".into(),
            ordering: "source".into(),
        },
        limits: Limits {
            max_structs_per_module: 1000,
            max_traits_per_module: 1000,
            max_impls_per_module: 1000,
        },
        ordering: Ordering {
            modules: vec![],
            structs: vec![],
...
fn collect_rs_files(root: &Path) -> Result<Vec<PathBuf>> {
    let mut out = Vec::new();
    // Walk entire workspace but skip target/.git to avoid non-source noise
