    ...
    for (bb_idx, bb) in body.basic_blocks.iter_enumerated() {
        ...
        if let TerminatorKind::Call { func, target, .. } = &terminator.kind {
            ...
            if let ty::FnDef(callee_def, _) = *func_ty.kind() {
                ...
                let mut edge = EdgePayload::new(caller_node, callee_node, EdgeKind::Call);
...
fn call_span<'tcx>(_tcx: TyCtxt<'tcx>, span: Span, _local_def: LocalDefId) -> Span {
    // For now we simply return the MIR span; future iterations may adjust to HIR spans.
...
fn serialize_cfg_graph<'tcx>(tcx: TyCtxt<'tcx>, body: &mir::Body<'tcx>) -> Option<String> {
    #[derive(Serialize)]
    struct SpanCapture {
        file: String,
        line: usize,
