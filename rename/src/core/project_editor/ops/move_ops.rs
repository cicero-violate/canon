...
pub(super) fn move_symbol_intra_file(ast: &mut syn::File, content: &str, handle: &NodeHandle, new_module_path: &str) -> Result<bool> {
    ...
    let (src_path, src_idx) = find_item_container_by_span(&ast.items, content, handle.byte_range).ok_or_else(|| anyhow::anyhow!("MoveSymbol: item not found by span"))?;
    let item = {
        let src_items = get_items_container_mut_by_path(&mut ast.items, &src_path).ok_or_else(|| anyhow::anyhow!("MoveSymbol: source container not found"))?;
        if src_idx >= src_items.len() {
            anyhow::bail!("MoveSymbol: item index out of bounds");
        }
        src_items.remove(src_idx)
    };
    let target_items = get_items_container_mut_by_path(&mut ast.items, &target_path).ok_or_else(|| anyhow::anyhow!("MoveSymbol: target mod path not found after extraction"))?;
