...
impl<'a> SymbolCollector<'a> {
    pub(super) fn new(module_path: &'a str, file: &'a Path, alias_graph: &'a mut crate::alias::AliasGraph) -> Self {
        Self { module_path, file, symbols: Vec::new(), current_impl: None, alias_graph }
    }
    ...
    fn add_symbol(&mut self, id: String, kind: &str, name: &str, span: Span, docs: Vec<String>, attrs: Vec<String>) {
    ...
    fn collect_use_tree_root(&mut self, tree: &syn::UseTree, docs: &[String], attrs: &[String], visibility: VisibilityScope) {
        self.collect_use_tree_with_prefix(tree, String::new(), docs, attrs, visibility);
    }

    fn collect_use_tree_with_prefix(&mut self, tree: &syn::UseTree, prefix: String, docs: &[String], attrs: &[String], visibility: VisibilityScope) {
        match tree {
            ...
            syn::UseTree::Name(name) => {
                ...
                let source_path = normalize_use_prefix(&parts, self.module_path).join("::");
                let local_name = name.ident.to_string();
                let id = format!("{}::use::{}", self.module_path, local_name);

                self.add_symbol(id.clone(), "use", &local_name, name.ident.span(), docs.to_vec(), attrs.to_vec());
            ...
            syn::UseTree::Rename(rename) => {
                ...
                self.add_symbol(id.clone(), "use", &local_name, rename.rename.span(), docs.to_vec(), attrs.to_vec());
            ...
            syn::UseTree::Glob(_glob) => {
                ...
                let source_path = normalize_use_prefix(&parts, self.module_path).join("::");
                let id = format!("{}::use::*::{}", self.module_path, source_path.replace("::", "_"));
            ...
            syn::UseTree::Group(group) => {
                ...
                for item in &group.items {
                    self.collect_use_tree_with_prefix(item, prefix.clone(), docs, attrs, visibility.clone());
                }
            }
        }
    }
...
impl<'ast> Visit<'ast> for SymbolCollector<'_> {
    ...
    fn visit_item_struct(&mut self, i: &'ast syn::ItemStruct) {
        ...
        let (docs, attrs) = Self::extract_docs_and_attrs(&i.attrs);
        self.add_symbol(id.clone(), "struct", &i.ident.to_string(), i.ident.span(), docs, attrs);
        for field in &i.fields {
            if let Some(ident) = &field.ident {
                let (field_docs, field_attrs) = Self::extract_docs_and_attrs(&field.attrs);
                let fid = format!("{}::{}", id, ident);
                self.add_symbol(fid, "field", &ident.to_string(), ident.span(), field_docs, field_attrs);
            }
        }
    ...
    fn visit_item_enum(&mut self, i: &'ast syn::ItemEnum) {
        let id = module_child_path(self.module_path, i.ident.to_string());
        let (docs, attrs) = Self::extract_docs_and_attrs(&i.attrs);
        self.add_symbol(id.clone(), "enum", &i.ident.to_string(), i.ident.span(), docs, attrs);
        for variant in &i.variants {
            let (variant_docs, variant_attrs) = Self::extract_docs_and_attrs(&variant.attrs);
            let vid = format!("{}::{}", id, variant.ident);
            self.add_symbol(vid.clone(), "variant", &variant.ident.to_string(), variant.ident.span(), variant_docs, variant_attrs);
    ...
    fn visit_item_trait(&mut self, i: &'ast syn::ItemTrait) {
        let id = module_child_path(self.module_path, i.ident.to_string());
        let (docs, attrs) = Self::extract_docs_and_attrs(&i.attrs);
        self.add_symbol(id.clone(), "trait", &i.ident.to_string(), i.ident.span(), docs, attrs);
        for item in &i.items {
            if let syn::TraitItem::Fn(method) = item {
                let (method_docs, method_attrs) = Self::extract_docs_and_attrs(&method.attrs);
                let mid = format!("{}::{}", id, method.sig.ident);
                self.add_symbol(mid, "trait_method", &method.sig.ident.to_string(), method.sig.ident.span(), method_docs, method_attrs);
            }
        }
    ...
    fn visit_item_fn(&mut self, i: &'ast syn::ItemFn) {
        ...
        let (docs, attrs) = Self::extract_docs_and_attrs(&i.attrs);
        self.add_symbol(id, "function", &i.sig.ident.to_string(), i.sig.ident.span(), docs, attrs);
    ...
    fn visit_item_type(&mut self, i: &'ast syn::ItemType) {
        let id = module_child_path(self.module_path, i.ident.to_string());
        let (docs, attrs) = Self::extract_docs_and_attrs(&i.attrs);
        self.add_symbol(id, "type", &i.ident.to_string(), i.ident.span(), docs, attrs);
    ...
    fn visit_item_const(&mut self, i: &'ast syn::ItemConst) {
        let id = module_child_path(self.module_path, i.ident.to_string());
        let (docs, attrs) = Self::extract_docs_and_attrs(&i.attrs);
        self.add_symbol(id, "const", &i.ident.to_string(), i.ident.span(), docs, attrs);
    ...
    fn visit_item_static(&mut self, i: &'ast syn::ItemStatic) {
        let id = module_child_path(self.module_path, i.ident.to_string());
        let (docs, attrs) = Self::extract_docs_and_attrs(&i.attrs);
        self.add_symbol(id, "static", &i.ident.to_string(), i.ident.span(), docs, attrs);
    ...
    fn visit_item_use(&mut self, i: &'ast syn::ItemUse) {
        ...
        let visibility = match &i.vis {
            ...
            syn::Visibility::Restricted(restricted) => VisibilityScope::Restricted(restricted.path.segments.iter().map(|s| s.ident.to_string()).collect::<Vec<_>>().join("::")),
    ...
    fn visit_impl_item_fn(&mut self, i: &'ast syn::ImplItemFn) {
        if let Some(ctx) = &self.current_impl {
            ...
            let (docs, attrs) = Self::extract_docs_and_attrs(&i.attrs);
            self.add_symbol(id, "method", &i.sig.ident.to_string(), i.sig.ident.span(), docs, attrs);
        }
