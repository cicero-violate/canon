...
impl AliasGraph {
    ...
    pub fn get_glob_imports(&self, module_path: &str) -> Vec<&ImportNode> {
        self.glob_imports
            .get(module_path)
            .map(|globs| globs.iter().filter_map(|(_, id)| self.nodes.get(id)).collect())
            .unwrap_or_default()
    }
    ...
    fn follow_reexport_chain_safe(
        ...
        while visited.insert(current.clone()) {
            ...
            let reexport = importers
                .iter()
                .find(|node| {
                    matches!(node.kind, UseKind::ReExport | UseKind::ReExportAliased)
                });
    ...
    fn find_reexport_chains_recursive(
        ...
        for reexport in reexports {
            ...
            let reexport_path = format!(
                "{}::{}", reexport.module_path, reexport.local_name
            );
