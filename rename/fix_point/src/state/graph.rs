...
impl GraphSnapshot {
    ...
    pub fn diff_nodes(&self, other: &GraphSnapshot) -> Vec<NodeId> {
        ...
        for id in left_nodes.keys().chain(right_nodes.keys()) {
            ...
            if left.key != right.key || left.label != right.label
        ...
        for id in left_edges.keys().chain(right_edges.keys()) {
            ...
            let left = left.unwrap();
            let right = right.unwrap();
            if left.from != right.from || left.to != right.to || left.kind != right.kind
                || left.metadata != right.metadata
            {
        ...
        pub fn reachable_from(&self, start: NodeId) -> Vec<NodeId> {
            ...
            for edge in &self.edges {
                if let (Some(&fi), Some(&ti)) = (
                    node_to_idx.get(&edge.from),
                    node_to_idx.get(&edge.to),
                ) {
                    adj[fi].push(ti);
                }
            }
...
impl std::fmt::Display for GraphDeltaError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            GraphDeltaError::Persistence(msg) => {
                write!(f, "graph persistence failed: {msg}")
            }
        }
    }
}
