...
impl<'a> ItemCollector<'a> {
    ...
    fn process_item(
        ...
        stack: &mut Vec<(Vec<syn::Item>, String, Option<ImplContext>)>,
    ) {
        match item {
            syn::Item::Mod(i) => {
            ...
            syn::Item::Impl(i) => {
                ...
                let trait_path = i
                    .trait_
                    .as_ref()
                    .map(|(_, path, _)| path_to_string(path, module_path));
                let ctx = ImplContext {
                    struct_path,
                    trait_path,
                };
                ...
                for ii in &i.items {
                    if let syn::ImplItem::Fn(f) = ii {
                        ...
                        let id = if let Some(tp) = &ctx.trait_path {
                            format!("{} as {}::{}", ctx.struct_path, tp, name)
                        } else {
        ...
        while let Some((tree, prefix)) = stack.pop() {
            match tree {
                ...
                syn::UseTree::Name(name) => {
                    ...
                    let source_path = normalize_use_prefix(&parts, module_path)
                        .join("::");
                ...
                syn::UseTree::Glob(_) => {
                    ...
                    let source_path = normalize_use_prefix(&parts, module_path)
                        .join("::");
                    let id = format!(
                        "{}::use::*::{}", module_path, source_path.replace("::", "_")
                    );
                    self.alias_graph
                        .add_use_node(ImportNode {
                            id,
