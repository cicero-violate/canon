...
fn collect_names_inner(project: &Path) -> Result<SymbolIndexReport> {
    ...
    let symbol_visibility: HashMap<String, VisibilityScope> = symbol_table
        ...
        .collect();
    let visibility_analysis = global_alias_graph
        .analyze_visibility_leaks(&symbol_visibility);
...
pub fn collect_symbols(
    ...
    let mut collector = ItemCollector::new(file, &mut alias_graph);
