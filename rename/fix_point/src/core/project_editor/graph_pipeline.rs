...
pub fn compare_snapshots(left: &GraphSnapshot, right: &GraphSnapshot) -> Result<()> {
    ...
    if left_map.len() != right_map.len() {
        ...
        let extra: Vec<_> = right_keys
            .difference(&left_keys)
            .take(10)
            .cloned()
            .collect();
        let missing: Vec<_> = left_keys
        ...
        return Err(
            anyhow!(
                ...
                left_map.len(), right_map.len(), extra, missing
            ),
        );
    }
    ...
    for edge in &left.edges {
        let (Some(from), Some(to)) = (
            left_id_to_key.get(&edge.from),
            left_id_to_key.get(&edge.to),
        ) else {
            continue;
        };
    ...
    for edge in &right.edges {
        let (Some(from), Some(to)) = (
            right_id_to_key.get(&edge.from),
            right_id_to_key.get(&edge.to),
        ) else {
            continue;
        };
...
pub fn emit_plan(
    ...
    for file in plan.files {
        let rel = file
            ...
            .map_err(|_| {
                anyhow!("plan path escapes project_root: {}", file.path.display())
            })?;
        let out_path = output_root.join(rel);
        let ast = syn::parse_file(&file.content)
            .map_err(|e| {
                let preview = file
                    .content
                    .lines()
    ...
    eprintln!(
        "Plan emission summary: written={}, unchanged={}, deletions={}", written.len(),
        unchanged.len(), deletion_count
    );
    Ok(EmissionReport {
        written,
        unchanged,
...
fn extract_item_snippet(
    ...
    if !matches!(
        node_kind, "struct" | "enum" | "union" | "trait" | "impl" | "function" | "const"
        | "static" | "type_alias"
    ) {
        return None;
    }
...
fn module_visibility(
    modules: &HashMap<String, WireNode>,
    module: &str,
) -> Result<String> {
...
pub fn project_plan(snapshot: &GraphSnapshot, project_root: &Path) -> Result<Plan1> {
    ...
    for node in &snapshot.nodes {
        ...
        *container_kind_counts.entry(container_kind).or_insert(0) += 1;
        ...
        if debug_plan && node.metadata.get("source_snippet").is_none() {
            eprintln!(
                "[plan] top-level item missing source_snippet: key={} kind={}", node.key,
                node_kind
            );
        }
    ...
    if debug_plan {
        ...
        eprintln!(
            "[plan] modules={} items_by_module={}", modules.len(), items_by_module.len()
        );
    }
    for module_path in module_list {
        ...
        if debug_plan {
            eprintln!(
                "[plan] module={} has_children={} file={}", module_path, has_children,
                file_path.display()
            );
        }
        ...
        if let Some(module_items) = items_by_module.get_mut(&module_path) {
            ...
            for node in module_items.drain(..) {
                ...
                if let Some(snippet) = snippet {
                    ...
                    if !snippet.is_empty() {
                        let vis = normalize_visibility(
                            node.metadata.get("visibility").map(|s| s.as_str()),
                        )?;
