...
pub(crate) fn run_pass1_canonical_rewrite(
    ...
    let (symbol_table, _occurrences, alias_graph) = build_symbol_index_and_occurrences(
        registry,
    )?;
        ...
        match tree {
            syn::UseTree::Name(name) => {
                ...
                if let Some((new_module, name)) = self.rewrite_map.get(&full) {
                    let mut new_segments: Vec<String> = new_module
                        .split("::")
                        .map(|s| s.to_string())
                        .collect();
                ...
                if let Some((new_module, name)) = self.rewrite_map.get(&full) {
                    ...
                    if let Some((first, rest)) = new_segments.split_first() {
                        ...
                        if let syn::UseTree::Name(name_tree) = &mut rebuilt {
                            ...
                            rebuilt = syn::UseTree::Rename(syn::UseRename {
                                ident: name_tree.ident.clone(),
                                as_token: rename.as_token,
                                rename: rename_ident,
                            });
                        }
                        *tree = rebuilt;
...
pub(crate) fn run_pass2_scope_rehydration(
    ...
    let project_root = registry
        ...
        .map(|p| p.to_path_buf())
        .unwrap_or_else(|| PathBuf::from("."));
    let dest_modules: HashSet<String> = moveset
        .entries
        .values()
        .map(|(_, new)| new.clone())
        .collect();
...
fn is_in_scope(
    alias_graph: &crate::alias::AliasGraph,
    module_path: &str,
    file: &str,
    local_name: &str,
    symbol_id: &str,
