...
pub fn assert_edge_endpoints_exist(snapshot: &GraphSnapshot) {
    ...
    for edge in &snapshot.edges {
        if !node_ids.contains(&edge.from) {
            panic!(
                "Invariant violation: edge.from references missing node id: {:?}", edge
                .from
            );
        }
        if !node_ids.contains(&edge.to) {
            panic!(
                "Invariant violation: edge.to references missing node id: {:?}", edge.to
            );
        }
    }
}
...
pub fn assert_module_path_consistency(snapshot: &GraphSnapshot) {
    ...
    for node in &snapshot.nodes {
        if let (Some(def_path), Some(module_path)) = (
            node.metadata.get("def_path"),
            node.metadata.get("module_path"),
        ) {
            if let Some(existing) = def_path_map.get(def_path) {
                if existing != module_path {
                    panic!(
                        "Invariant violation: def_path {} appears in multiple modules: {} vs {}",
                        def_path, existing, module_path
                    );
                }
            } else {
...
pub fn assert_no_duplicate_edges(snapshot: &GraphSnapshot) {
    ...
    for edge in &snapshot.edges {
        ...
        if !seen.insert(key.clone()) {
            panic!(
                "Invariant violation: duplicate edge detected: from={:?} to={:?} kind={}",
                key.0, key.1, key.2
            );
        }
    }
