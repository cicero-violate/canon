...
fn propagate_move(
    ...
    if let Some(new_crate) = new_crate {
        conflicts
            .push(EditConflict {
                ...
                reason: format!(
                    "cross-crate move requires Cargo.toml update ({new_crate})"
                ),
            });
    ...
    let old_module_path = normalize_symbol_id(
    ...
    if let Some(old_module) = parent_module_path(&norm_id) {
        if old_module != new_module_path {
            for occ in occurrences
                .iter()
                .filter(|occ| {
                    occ.id == old_module && occ.kind == "use_path"
...
fn push_unique_edit(
    ...
    let key = format!(
        "{}:{}:{}:{}:{}:{}", occ.file, occ.span.start.line, occ.span.start.column, occ
        .span.end.line, occ.span.end.column, new_name
    );
    if seen.insert(key) {
...
fn propagate_visibility(
    ...
    let mut conflicts: Vec<EditConflict> = oracle
        ...
        .collect();
    let (_symbol_table, _occurrences, alias_graph) = build_symbol_index_and_occurrences(
        registry,
    )?;
...
pub(crate) fn build_symbol_index_and_occurrences(
    registry: &NodeRegistry,
    ...
    for (file, ast) in &registry.asts {
        ...
        let mut visitor = OccurrenceVisitor::new(
            &module_path,
            file,
            &symbol_table,
            &use_map,
...
pub fn propagate(
    ...
    match op {
        ...
        NodeOp::MoveSymbol { new_module_path, new_crate, .. } => {
            propagate_move(
                symbol_id,
                new_module_path,
                new_crate.as_deref(),
...
fn propagate_rename(
    ...
    let (_symbol_table, occurrences, _alias_graph) = build_symbol_index_and_occurrences(
        registry,
    )?;
    let mut rewrites = Vec::new();
    ...
    Ok(PropagationResult {
        ...
        conflicts,
        file_renames: {
            let mapping: HashMap<String, String> = std::iter::once((
                    norm_id.clone(),
                    new_name.to_string(),
                ))
                .collect();
