...
fn main() -> Result<()> {
    ...
    let repo_root = PathBuf::from(env!("CARGO_MANIFEST_DIR")).parent().ok_or_else(|| anyhow!("cannot resolve host_runtime parent"))?.to_path_buf();
    match parse_cli_mode()? {
        CliMode::Shell(options) => run_shell_flow(&repo_root, options),
        CliMode::Execute { verify_only, intent } => run_system_flow(&repo_root, verify_only, intent),
    }
}
