...
pub fn reaching_definitions(
    ...
    while changed {
        changed = false;
        for &b in blocks {
            let new_in: HashSet<DefId> = pred.get(&b)
                .map(|ps| ps.iter().flat_map(|p| out[p].iter().cloned()).collect())
                .unwrap_or_default();
            let kill = facts.kill.get(&b).cloned().unwrap_or_default();
            let r#gen = facts.r#gen.get(&b).cloned().unwrap_or_default();
            let new_out: HashSet<DefId> = r#gen.union(
                &new_in.difference(&kill).cloned().collect::<HashSet<_>>()
            ).cloned().collect();
            if new_out != out[&b] {
