...
pub fn validate_delta(delta: &Delta) -> Result<(), DeltaError> {
    if delta.is_sparse {
        ...
        if changed != delta.payload.len() {
            return Err(DeltaError::SizeMismatch { mask_len: changed, payload_len: delta.payload.len() });
        }
    } else if delta.mask.len() != delta.payload.len() {
        return Err(DeltaError::SizeMismatch { mask_len: delta.mask.len(), payload_len: delta.payload.len() });
    }
