...
impl PageAllocator {
    pub fn new(default_location: PageLocation) -> Self {
        Self { pages: Arc::new(Mutex::new(HashMap::new())), default_location }
    }

    pub fn from_config(config: PageAllocatorConfig) -> Self {
        Self { pages: Arc::new(Mutex::new(HashMap::with_capacity(config.initial_capacity))), default_location: config.default_location }
    }

    pub fn with_page<F, R>(&self, id: PageID, f: F) -> Option<R>
    where F: FnOnce(&Page) -> R {
        let pages = self.pages.lock();
        pages.get(&id).map(f)
    }

    pub fn with_page_mut<F, R>(&self, id: PageID, f: F) -> Option<R>
    where F: FnOnce(&mut Page) -> R {
        let mut pages = self.pages.lock();
        pages.get_mut(&id).map(f)
    }

    pub fn snapshot(&self, id: PageID) -> Option<PageSnapshotData> {
        self.with_page(id, |page| PageSnapshotData { page_id: id, data: page.data_slice().to_vec(), location: page.location(), epoch: page.epoch() })
    }
    ...
    pub fn snapshot_pages(&self) -> Vec<PageSnapshotData> {
        let pages = self.pages.lock();
        pages.iter().map(|(id, page): (&PageID, &Page)| PageSnapshotData { page_id: *id, data: page.data_slice().to_vec(), location: page.location(), epoch: page.epoch() }).collect()
    }
