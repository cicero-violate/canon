...
__device__ void sha256_page(
    ...
    uint64_t total_bits = PAGE_SIZE * 8ULL;

    for (int chunk = 0; chunk < PAGE_SIZE; chunk += 64) {
        #pragma unroll
        for (int i = 0; i < 64; i++) {
            block[i] = data[chunk + i];
        }

        // Padding in final block
        if (chunk == PAGE_SIZE - 64) {
            block[63] = 0x80;
        }
...
extern "C"
void launch_hash_leaves_and_rebuild(
    uint8_t* tree,
    uint64_t tree_size,
    const uint8_t* pages
) {
